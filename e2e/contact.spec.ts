...
test('contact page leads with capture and world-model requests', async ({ page }) => {
  ...
  await expect(page.getByRole('link', { name: /Start robot-team brief/i })).toBeVisible();
  ...
  await expect(page.getByText(/Brief first\. Calendar second\./i)).toBeVisible();
  await expect(page.getByText(/Required to route/i)).toBeVisible();
});
