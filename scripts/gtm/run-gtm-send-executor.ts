...
function renderReport(input: {
  ...
  return [
    ...
    "## Gate Summary",
    "",
    `- recipient_backed_targets: ${input.result.gateSummary.recipientBackedTargets}`,
    `- proof_ready_outreach_targets: ${input.result.gateSummary.proofReadyOutreachTargets}`,
    `- demand_sourced_capture_targets: ${input.result.gateSummary.demandSourcedCaptureTargets}`,
    `- proof_artifacts_or_capture_asks: ${input.result.gateSummary.proofArtifactsOrCaptureAsks}`,
    `- approval_ready_targets: ${input.result.gateSummary.approvalReadyTargets}`,
    `- approval_blockers: ${input.result.gateSummary.approvalBlockers}`,
    `- reply_durability_blocked_targets: ${input.result.gateSummary.replyDurabilityBlockedTargets}`,
    `- stale_next_action_targets: ${input.result.gateSummary.staleNextActionTargets}`,
    `- stale_blocker_projection_targets: ${input.result.gateSummary.staleBlockerProjectionTargets}`,
    "",
    "## Receipts",
    "",
