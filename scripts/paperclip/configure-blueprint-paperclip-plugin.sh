...
secret_id_by_name() {
  local company_id="$1"
  local secret_name="$2"
  curl -fsS "${PAPERCLIP_PUBLIC_URL}/api/companies/${company_id}/secrets" \
...
upsert_secret_from_env() {
  ...
  if [ -n "$existing_id" ]; then
    curl -fsS -X POST \
      ...
      "${PAPERCLIP_PUBLIC_URL}/api/secrets/${existing_id}/rotate" >/dev/null
    printf '%s' "$existing_id"
    return 0
  fi
  ...
  curl -fsS -X POST \
    ...
    "${PAPERCLIP_PUBLIC_URL}/api/companies/${company_id}/secrets" \
...
ensure_plugin_installed() {
  ...
  if plugin_record="$(plugin_json 2>/dev/null)"; then
    ...
    if [ "$status" != "ready" ]; then
      curl -fsS -X POST "${PAPERCLIP_PUBLIC_URL}/api/plugins/${PLUGIN_KEY}/enable" >/dev/null || true
    fi
    return 0
  fi
  ...
  curl -fsS -X POST \
    ...
    "${PAPERCLIP_PUBLIC_URL}/api/plugins/install" >/dev/null
}
...
write_plugin_config() {
  ...
  curl -fsS -X POST \
    -H "Content-Type: application/json" \
    -d "$payload" \
    "${PAPERCLIP_PUBLIC_URL}/api/plugins/${plugin_id}/config" >/dev/null
}
