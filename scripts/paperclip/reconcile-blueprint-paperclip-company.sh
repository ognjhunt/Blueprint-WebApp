...
BLUEPRINT_PAPERCLIP_HERMES_PRIMARY_MODEL="${BLUEPRINT_PAPERCLIP_HERMES_PRIMARY_MODEL:-deepseek-v4-pro[1m]}"
BLUEPRINT_PAPERCLIP_HERMES_FALLBACK_MODEL="${BLUEPRINT_PAPERCLIP_HERMES_FALLBACK_MODEL:-deepseek-v4-flash}"
...
const DEFAULT_HERMES_MODEL = "deepseek-v4-pro[1m]";
const DEFAULT_HERMES_FALLBACK_MODEL = "deepseek-v4-flash";
...
const hermesDirectProvider =
  configuredHermesProvider.length > 0
    && configuredHermesProvider !== DEFAULT_HERMES_OPENROUTER_PROVIDER
    ? process.env.BLUEPRINT_PAPERCLIP_HERMES_PROVIDER.trim()
...
function buildInvestorRelationsRoutineDescription() {
  return [
    ...
    "When the active Resend-backed email draft path is configured, maintain the monthly investor draft. Do not live send or publish.",
