...
Then, unless --prepare-only is passed, runs:
  scripts/graphify/run-staged-ast-pilot.py

Options:
  --prepare-only   Stage the corpus only; do not run graphify
  --publish-only   Copy the latest derived pilot artifacts back to root graphify-out without restaging
...
if [ -f "$GRAPHIFY_IGNORE_SOURCE" ]; then
  # The root ignore excludes derived/graphify/ so full-repo scans skip generated
  # graph artifacts. This staged corpus intentionally lives under that path, so
  # omit that one pattern in the copied ignore file or graphify will ignore the
  # entire pilot corpus.
  sed '/^[[:space:]]*derived\/graphify\/[[:space:]]*$/d' "$GRAPHIFY_IGNORE_SOURCE" > "$CORPUS_DIR/.graphifyignore"
fi
...
# repo root intentionally ignores for full-repo scans. Run detection against an
...
echo "[graphify pilot] derived outputs are under $CORPUS_DIR/graphify-out"
echo "[graphify pilot] canonical root report: $ROOT_OUTPUT_DIR/GRAPH_REPORT.md"
if [ -f "$ROOT_OUTPUT_DIR/wiki/index.md" ]; then
  echo "[graphify pilot] canonical root wiki index: $ROOT_OUTPUT_DIR/wiki/index.md"
else
  echo "[graphify pilot] note: no wiki/index.md was generated by this AST pilot run"
fi
