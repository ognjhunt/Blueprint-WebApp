...
const AGENT_KNOWLEDGE_ENTRIES: AgentKnowledgeEntry[] = [
  ...
  },
  {
    ...
    answer:
      "An agent with a budget buys through POST /api/agent-access/commerce/live-checkout. Send siteWorldId, product (hosted_session_rental or site_world_package), optional sessionHours, and optional budgetCents; the server prices the SKU itself and refuses with a structured budget_exceeded blocker if the quote is over budget. Eligible requests return a real Stripe Checkout URL plus an order id. After the payment method on file completes Stripe checkout, webhook fulfillment marks the order paid and provisions a marketplace entitlement automatically — poll GET /api/agent-access/commerce/live-orders/{orderId} until provisioned. Live checkout is only offered for pipeline-backed site worlds; sample or planned catalog profiles return a not_live_purchasable blocker with a request-intake alternative.",
  ...
  },
  {
    ...
    answer:
      "Every agent surface carries truth labels: capture_grounded, provider_derived, generated, sample_demo, public_demo_eligible, request_gated, protected_robot_team, dry_run_order, and live_checkout. Ground truth means raw capture evidence, provenance, rights/privacy records, and runtime artifacts. Samples, generated previews, dry-run commerce, catalog matches, and request drafts are support signals — they do not prove customer results, rights clearance, provider execution, payment, or deployment readiness. A paid live order proves payment and entitlement provisioning only.",
