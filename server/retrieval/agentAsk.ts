...
const AGENT_KNOWLEDGE_ENTRIES: AgentKnowledgeEntry[] = [
  ...
  },
  {
    ...
    answer:
      "One published price, paid by one side. A site pays nothing to find out: the task defined, the site captured, candidates screened, the final comparison run for a shortlist of up to five candidates, and a pilot recommendation, all at no charge and with no card. A site records one walkthrough on a phone; what it is not given free is a physical pilot, which is quoted separately when there is something to buy rather than something to learn. Robot teams pay for evaluation against real sites, and that is what funds the free side. They never receive the site's recording: the walkthrough is reconstructed into a 3D scene, and the scene is the environment their policies are evaluated in, under the rights the site granted at intake and nothing wider. A robot team pays $99 per entry, flat. One entry is one policy, running on one embodiment, against one task at one site: the same policy on a second embodiment is a second entry, a second policy on the same embodiment is a second entry, and a different embodiment running a different policy is one new entry rather than two. The bill is entries times tasks times $99 and has no other term, so three policies on one task is $297, three policies on two tasks is $594, and one policy on three tasks is $297. Blueprint decides how much evaluation an entry gets and runs it, under the same conditions for every entry on a task, which is why no team can buy a longer run than a rival and why a shortlisted entry pays nothing more. There is no per-episode rate, subscription, listing fee, submission fee, seat fee, or percentage of any contract. The full quote is shown and reserved before a run starts and anything that does not run is released. A failed attempt is billable; an environment that will not launch is not. The server-side quote is authoritative. A run may return a bounded positive or negative decision, a partial decision, an explicit abstention, or the next evidence required; a comparison may come back too close to separate, simulated ranking is not real-world ranking, and nothing here guarantees a ranking, winner, deployment, or pilot outcome.",
