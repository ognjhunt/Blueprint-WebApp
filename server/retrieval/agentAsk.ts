...
const AGENT_KNOWLEDGE_ENTRIES: AgentKnowledgeEntry[] = [
  ...
  },
  {
    ...
    answer:
      "One published price, paid by one side. A site pays nothing to find out: the task defined, the site captured, candidates screened, the finalist comparison run for a shortlist of up to five candidates, and a pilot recommendation, all at no charge and with no card. A site records one walkthrough on a phone; what it is not given free is a physical pilot, which is quoted separately when there is something to buy rather than something to learn. Robot teams pay for screening against real sites, and that is what funds the free side. They never receive the site's recording: the walkthrough is reconstructed into a 3D scene, and the scene is the environment their policies are evaluated in, under the rights the site granted at intake and nothing wider. A robot team pays $0.50 per standard episode and buys exactly one thing: screening its own checkpoints at 50 episodes each, so $25 per checkpoint. Screening only cuts a candidate it can rule out, roughly 20 points behind the leader at that count; everything it cannot separate from the leader advances, up to five. One episode is one run of one policy on one scenario up to the task's published time limit, with a result, so six checkpoints on the same scenario is six episodes. A shortlisted team pays nothing more; the 500-episode finalist round is funded and run by Blueprint so every finalist gets the same episodes under the same conditions, which is also why no team can buy a longer run than a rival. The full quote is shown and reserved before a run starts, unrun episodes are released, and a non-standard workload is quoted at its own visible per-episode rate. There is no subscription, listing fee, submission fee, or percentage of any contract. The episode counts are Blueprint operating budgets chosen by what they resolve, not an industry standard, and simulated ranking is not real-world ranking. The server-side quote is authoritative. A run may return a bounded positive or negative decision, a partial decision, an explicit abstention, or the next evidence required; it never guarantees a ranking, winner, deployment, or pilot outcome.",
