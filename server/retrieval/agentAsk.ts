...
const AGENT_KNOWLEDGE_ENTRIES: AgentKnowledgeEntry[] = [
  ...
  },
  {
    ...
    answer:
      "Two published prices, paid by two different parties. A site pays $2,500 once per site-task for a scoped assessment: the task and its pass mark defined, the site captured, candidates screened, and a pilot recommendation. A robot team pays $0.50 per standard episode from a prepaid dollar balance, where one episode is one run of one policy on one scenario up to the task's published time limit, with a result — so six checkpoints on the same scenario is six episodes. The full checkpoints x episodes quote is shown and reserved before a run starts, unrun episodes are released, and a non-standard workload is quoted at its own visible per-episode rate. There is no subscription, listing fee, submission fee, or percentage of any contract. The server-side quote is authoritative. A run may return a bounded positive or negative decision, a partial decision, an explicit abstention, or the next evidence required; it never guarantees a ranking, winner, deployment, or pilot outcome.",
