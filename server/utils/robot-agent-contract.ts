...
export function buildRobotAgentOpenApiContract() {
  return {
    ...
    tags: [
      ...
      { name: "Team", description: "Register a robot team, plan evaluations, and buy runs. Register needs no credential; everything spend-shaped needs the team's agent key." },
    ...
    paths: {
      ...
      "/api/agent-team/register": {
        post: {
            ...
            content: {
              "application/json": {
                schema: {
                  ...
                  required: ["teamName"],
                  properties: {
                    teamName: { type: "string", minLength: 2, maxLength: 120 },
                    ...
                    checkpoint: {
                      ...
                      properties: {
                        ...
                        runtime: {
                          type: "string",
                          enum: ["endpoint", "container", "artifact"],
                        },
                        reference: {
                          type: "string",
                          description: "A URL, an image reference, or an artifact location.",
                        },
                      },
                    },
                  },
          ...
          responses: {
            "201": {
              description:
                "Team registered. `agentKey` is returned once; funding and a spend policy are required before any run is bought.",
              content: {
                "application/json": { schema: { type: "object" } },
              },
            },
            "400": jsonResponse("#/components/schemas/ErrorResponse", "Invalid registration"),
            "429": jsonResponse("#/components/schemas/ErrorResponse", "Registration rate limited"),
          },
        },
      },
      "/api/agent-team/keys/reissue": {
        post: {
          tags: ["Team"],
          ...
          responses: {
            "202": {
              description: "Accepted. Keys, if any, are delivered by email to the registered address.",
              content: {
                "application/json": { schema: { type: "object" } },
              },
            },
            "400": jsonResponse("#/components/schemas/ErrorResponse", "Invalid request"),
            "429": jsonResponse("#/components/schemas/ErrorResponse", "Re-issue rate limited"),
          },
        },
      },
      "/api/agent-team/plan": {
        post: {
          ...
          responses: {
            "200": {
              description: "A ranked plan with per-run costs. Nothing is committed.",
              content: {
                "application/json": { schema: { type: "object" } },
              },
            },
      ...
      "/api/agent-team/runs": {
        post: {
                ...
                schema: {
                  ...
                  properties: {
                    confirm: {
                      type: "boolean",
                      default: false,
                      description: "True to commit spend. Omit for a dry run.",
                    },
          ...
          responses: {
            "200": {
              description: "Run accepted (or the priced dry run when confirm is omitted).",
              content: {
                "application/json": { schema: { type: "object" } },
              },
            },
