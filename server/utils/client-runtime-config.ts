...
/** Numeric segment-wise compare of dotted versions: negative when a < b. */
export function compareDottedVersions(a: string, b: string): number {
  const left = a.trim().split(".").map((part) => Number(part) || 0);
  const right = b.trim().split(".").map((part) => Number(part) || 0);
...
/**
 * A client with an unparseable/absent version does NOT satisfy a configured
 * minimum (fail closed for enforcement), but every client satisfies the
 * "0.0.0" no-floor default.
 */
export function clientVersionSatisfiesMinimum(
  clientVersion: string | null | undefined,
  minSupportedVersion: string,
): boolean {
  if (compareDottedVersions(minSupportedVersion, "0.0.0") === 0) {
    return true;
  }
  if (!isValidVersionString(clientVersion)) {
    return false;
  }
  return compareDottedVersions(clientVersion, minSupportedVersion) >= 0;
}
