...
export type CityLaunchExecutionResult = {
  ...
  notionSyncStatus?: "synced" | "skipped_not_configured" | "failed";
  notionSyncError?: string | null;
  paperclipUpdateStatus?: "dispatched" | "degraded" | "skipped" | "failed";
  paperclip?: {
...
function renderLaunchSurfaceCoverageRows(
  ...
  return [
    ...
    "|---|---|---|---|---|---|",
    ...coverage.map((entry) =>
      `| ${entry.surfaceKey} | ${laneDisplayName(entry.ownerLane)} | ${entry.humanLane ? laneDisplayName(entry.humanLane) : "none"} | ${entry.artifact} | ${entry.completionGate}${entry.validationRequired ? " (validation required)" : ""} | ${entry.blockerBehavior} |`,
    ),
  ];
}
...
function buildCityOpeningBriefMarkdown(input: {
  ...
  return [
    ...
    "",
    "## Core Message",
    "- Who Blueprint is: a capture-first company turning real site-tasks into maintained testbeds and Task Evaluation Runs with explicit decisions, abstentions, and proof boundaries.",
...
function buildIndoorLocationSupplyPlaceholderArtifacts(input: {
  profile: CityLaunchProfile;
  ...
  const base = {
    ...
    source_activation_payload: input.sourceActivationPayloadPath,
    delegation_task_key: "public-commercial-community-sourcing",
    evidence_boundary:
      "Placeholder blocker artifact only; no location is approved, rights-cleared, payable, or derived-world-model-ready until evidence review passes.",
  };
  const supply = {
    schema_version: "2026-04-25.indoor-location-supply.v1",
    ...base,
    accepted_candidates: [],
    blocker_behavior:
      "capturer-growth-agent must run indoor public-location supply sourcing and seed review-only candidates with evidence before this lane can close.",
  };
  const rejected = {
    schema_version: "2026-04-25.indoor-location-supply-rejected.v1",
    ...base,
    rejected_candidates: [],
  };
  const evidenceLog = {
    schema_version: "2026-04-25.indoor-location-supply-evidence-log.v1",
    ...base,
    source_queries: [],
    fetched_source_urls: [],
    evidence_events: [],
  ...
  const report = [
    `# ${input.profile.city} Indoor Location Supply Report`,
    "",
    "- status: awaiting_capturer_growth_agent_location_supply",
    `- source_activation_payload: \`${input.sourceActivationPayloadPath}\``,
    "- delegation_task_key: `public-commercial-community-sourcing`",
  ...
  return { supply, rejected, evidenceLog, report };
}
...
export function buildCityExecutionTasks(profile: CityLaunchProfile): CityLaunchTask[] {
  return [
    ...
    },
    {
      ...
      phase: "supply",
      title: `Run ${profile.shortLabel} indoor public-location supply sourcing`,
      ownerLane: "capturer-growth-agent",
      humanLane: "growth-lead",
      ...
      inputs: [
        ...
        `city-launch-${profile.key}-deep-research.md`,
        profile.launchPlaybookPath,
        `${profile.shortLabel} source policy`,
        `${profile.shortLabel} city-opening brief`,
        `${profile.shortLabel} first-wave posting pack`,
        `ops/paperclip/playbooks/city-launch-${profile.key}-indoor-location-supply.json`,
        `ops/paperclip/playbooks/city-launch-${profile.key}-indoor-location-supply-rejected.json`,
        `ops/paperclip/playbooks/city-launch-${profile.key}-indoor-location-supply-evidence-log.json`,
        `ops/paperclip/playbooks/city-launch-${profile.key}-indoor-location-supply-report.md`,
      ...
      doneWhen: [
        `${profile.shortLabel} indoor supply names a broad set of concrete indoor-only or indoor-primary public-facing/common-access location candidates with source URLs, source queries, public-access posture, indoor posture, allowed capture zones, avoid zones, confidence, verification status, and review-only or promotion-ready state.`,
        `${profile.shortLabel} rejected candidates are saved with reason codes and evidence instead of silently disappearing.`,
        `${profile.shortLabel} evidence log records the search queries and fetched URLs used after reusing the city Deep Research artifact as a source map.`,
        `${profile.shortLabel} review candidates are validated with the seed script, materialized into a durable candidate ledger or app-visible review feed, and automatically passed through deterministic public-space review for the newly written ids without marking them rights-cleared, operator-approved, payable, or derived-world-model-ready.`,
...
function buildSystemDocMarkdown(input: {
  ...
  return [
    ...
    "- doctrine: capture-first, real-site Task Evaluation Run as the single product, with Pipeline-owned evidence routing and verdicts",
    "",
    "## Objective",
    "",
...
function buildCityLaunchRootBlockerSummaryComment(input: {
  ...
  const formatGroup = (entries: CityLaunchTaskDispatch[]) =>
    entries.length > 0
      ...
      : ["  - none"];
  const wakeBlocked = input.dispatched.filter((entry) => entry.wakeError);
  ...
  return [
    ...
    "",
    `- Wake blockers (${wakeBlocked.length}):`,
    ...(wakeBlocked.length > 0
      ? wakeBlocked.map((entry) =>
          `  - ${renderDispatchLink(entry)}: ${entry.wakeStatus || "failed"} — ${entry.wakeError}`,
        )
      : ["  - none"]),
    "",
...
function buildTaskKickoffComment(input: {
  ...
  return [
    ...
    ...(input.task.key === "public-commercial-community-sourcing"
      ? [
          "Build public-facing location supply first: name concrete review-only locations, record source URLs and capture/avoid zones, and materialize them into the candidate ledger or app review feed without waiting for a live community reply.",
        ]
      : []),
...
export async function runCityLaunchExecutionHarness(input: {
  ...
  const indoorLocationSupplyPath = path.join(runDirectory, `city-launch-${profile.key}-indoor-location-supply.json`);
  ...
  try {
    ...
    if (autonomousActivation && planningState.status !== "completed") {
      throw new Error(
        `${profile.city} autonomous activation requires the latest city planning state to be completed; current state is ${planningState.status}.`,
      );
    }
    ...
    const indoorLocationSupplyArtifacts = buildIndoorLocationSupplyPlaceholderArtifacts({
      profile,
      generatedAtIso: startedAt.toISOString(),
      sourceActivationPayloadPath: completedResearch?.activationPayload
        ? canonicalActivationPayloadPath
        : "missing_activation_payload",
    });
    await writeTextArtifact(
      indoorLocationSupplyPath,
      JSON.stringify(indoorLocationSupplyArtifacts.supply, null, 2),
    );
    await writeTextArtifact(
      indoorLocationSupplyRejectedPath,
      JSON.stringify(indoorLocationSupplyArtifacts.rejected, null, 2),
    );
    await writeTextArtifact(
      indoorLocationSupplyEvidenceLogPath,
      JSON.stringify(indoorLocationSupplyArtifacts.evidenceLog, null, 2),
    );
    await writeTextArtifact(indoorLocationSupplyReportPath, indoorLocationSupplyArtifacts.report);
    await writeTextArtifact(
      canonicalIndoorLocationSupplyPath,
      JSON.stringify(indoorLocationSupplyArtifacts.supply, null, 2),
    );
    await writeTextArtifact(
    ...
    const issueBundlePayload = {
      ...
      launch_surface_coverage:
        completedResearch?.activationPayload?.launchSurfaceCoverage?.map((entry) => ({
          ...
          owner_lane: entry.ownerLane,
          human_lane: entry.humanLane,
          artifact: entry.artifact,
          evidence_standard: entry.evidenceStandard,
          completion_gate: entry.completionGate,
          delegation_task_key: entry.delegationTaskKey,
          blocker_behavior: entry.blockerBehavior,
          validation_required: entry.validationRequired,
    ...
    const result: CityLaunchExecutionResult = {
      ...
      artifacts: {
        ...
        indoorLocationSupplyArtifacts: {
          run: {
            ...
            rejectedPath: indoorLocationSupplyRejectedPath,
            evidenceLogPath: indoorLocationSupplyEvidenceLogPath,
            reportPath: indoorLocationSupplyReportPath,
          },
          canonical: {
            supplyPath: canonicalIndoorLocationSupplyPath,
            rejectedPath: canonicalIndoorLocationSupplyRejectedPath,
            evidenceLogPath: canonicalIndoorLocationSupplyEvidenceLogPath,
            reportPath: canonicalIndoorLocationSupplyReportPath,
          },
        },
      },
      planning: {
      ...
      notionSyncStatus: getNotionToken() ? "failed" : "skipped_not_configured",
      notionSyncError: null,
      paperclipUpdateStatus: input.dispatchIssues === false ? "skipped" : "failed",
    };

    await advanceStep("sync_notion");
    let notion: Awaited<ReturnType<typeof syncExecutionArtifactsToNotion>> = null;
    try {
      notion = await syncExecutionArtifactsToNotion({
        profile,
        status,
        canonicalSystemDocPath,
        systemDocText,
      ...
      result.notionSyncStatus = notion ? "synced" : "skipped_not_configured";
    } catch (error) {
      result.notionSyncStatus = "failed";
      result.notionSyncError = error instanceof Error ? error.message : String(error);
    }
