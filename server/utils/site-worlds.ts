...
function buildFallbackPackages(siteId: string, siteName: string, siteAddress: string, sampleTask: string, sampleRobot: string): [SiteWorldPackage, SiteWorldPackage] {
  ...
  return [
    ...
    },
    {
      ...
      summary: "Open Blueprint-managed policy evaluation setup when this site is ready for downstream testing.",
      priceLabel: "$6,500 / site evaluation",
      payerLabel: "Likely buyer: Robot team",
      actionLabel: "Open policy evaluation setup",
      actionHref: `/site-worlds/${siteId}/start`,
      deliverables: [
        "One site and one robot policy/profile",
        "Task suite and scenario variants",
        "Rollout exports and logs",
        "Episode summaries and failure notes",
      ],
    },
  ];
}
