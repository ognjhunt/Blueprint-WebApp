...
describe("Home", () => {
  ...
  it("surfaces honest proof, deliverables, and trust", { timeout: 10000 }, () => {
    ...
    expect(
      screen.getByRole("link", { name: /See deliverables and technical reference/i }),
    ).toHaveAttribute(
      "href",
      "/sample-deliverables",
    );
    expect(
      screen.getByText(
        /Blueprint captures real facilities and turns them into products robot teams can buy\./i,
      ),
    ).toBeInTheDocument();
  });
});
