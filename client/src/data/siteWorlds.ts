...
function buildPackages(config: PackageConfig): [SiteWorldPackage, SiteWorldPackage] {
  return [
    ...
    },
    {
      ...
      summary:
        "Request a Blueprint-managed evaluation set for this exact site when your team wants one policy/profile run against one scoped task pack after setup verifies account, package, and runtime availability.",
      priceLabel: config.hostedRate,
      payerLabel: "Robot team",
      actionLabel: "Request policy evaluation",
      actionHref: buildContactHref("hosted-evaluation", config),
      deliverables: [
        "One site, one robot policy/profile, and one scoped task pack",
        "Up to 500 scenarios/episodes",
        "Pass/fail metrics, cycle time, intervention and failure notes",
        "Scenario/results manifest and policy comparison on the same site",
      ],
    },
  ];
}
...
const rawSiteWorldCards: RawSiteWorldCard[] = [
  {
    ...
    packages: buildPackages({
      ...
      siteAddress: "1847 W Fulton St, Chicago, IL 60612",
      scenePrice: "$2,400",
      hostedRate: "From $6,500 / Policy Evaluation Run",
      sampleTask: "Walk to shelf staging and pick the blue tote",
      sampleRobot: "Unitree G1 with head cam and wrist cam",
    }),
  },
