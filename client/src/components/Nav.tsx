...
export default function Nav({ blueprintTitle }: NavProps) {
  ...
  return (
    ...
    >
      {/* Container - Full width with controlled padding on left/right */}
      <div className="flex items-center justify-between h-full w-full px-6 md:px-12 lg:px-16">
        {/* LEFT: Brand logo and name - positioned further left */}
        ...
        <div className="hidden md:flex items-center">
          ...
          {currentUser && (
            <>
              ...
              {/* Updated Invite button with proper spacing and link to workspace */}
                ...
                <DropdownMenuTrigger asChild>
                  ...
                  >
                    <Avatar className="h-10 w-10">
                      {/* Use userData.photoURL if available */}
                      <AvatarImage
                        src={userData?.photoURL || ""}
                        alt={userData?.name || "User Profile"}
                      />
                      {/* Fallback shows initials if name exists, otherwise User icon */}
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            className="md:hidden absolute top-full left-0 right-0 bg-white/95 backdrop-blur-md shadow-lg rounded-b-2xl border-t border-indigo-50"
          ...
          >
            <div className="p-6 flex flex-col space-y-5">
              ...
              <Link href="/pricing" className="w-full">
                <div className="text-gray-700 hover:text-indigo-600 text-sm font-medium py-3 px-4 hover:bg-indigo-50/50 rounded-xl transition-colors duration-200">
                  Pricing
                </div>
              </Link>
              ...
              {currentUser && (
                <>
                  <Link href="/scanner-portal" className="w-full">
                    <div className="text-gray-700 hover:text-indigo-600 text-sm font-medium py-3 px-4 hover:bg-indigo-50/50 rounded-xl transition-colors duration-200">
                      Scanner Portal
                    </div>
                  </Link>
              ...
              {!currentUser ? (
                <Link href="/sign-in" className="w-full">
                  <Button className="w-full py-6 rounded-xl bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-700 hover:to-violet-700 text-white shadow-md">
                    Sign In
                  </Button>
                </Link>
              ) : (
                <>
                  <Link href="/dashboard" className="w-full">
                    <Button
                      variant="outline"
                      className="w-full py-6 rounded-xl border-indigo-200"
                    >
                      Dashboard
                    </Button>
                  </Link>
                  ...
                  <Button
                    variant="outline"
                    className="w-full py-6 rounded-xl border-red-200 text-red-600 hover:bg-red-50 hover:text-red-700"
                    onClick={handleSignOut}
                  >
                    Sign Out
                  </Button>
