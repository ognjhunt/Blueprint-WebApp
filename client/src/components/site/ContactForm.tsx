      ...
      {requestType === "dataset" ? (
        ...
        <section className="space-y-8">
          <div className="space-y-4">
            <div className="space-y-2">
              <span className="text-xs uppercase tracking-[0.3em] text-slate-400">SimReady location capture</span>
              <h3 className="text-xl font-semibold text-slate-900">On-site SimReady Location (waitlist)</h3>
              <p className="text-sm text-slate-600">
                Turn a real site into a validated digital twin. Whether you need to capture a facility you already control or a
                prospect’s floor you hope to deploy into, we scan, rebuild, and deliver SimReady scenes within days so your
                robotics team can prove ROI in simulation before rolling out hardware.
              </p>
            </div>
            <div className="space-y-3 rounded-3xl border border-slate-200 bg-slate-50 p-5">
              <h4 className="text-sm font-semibold text-slate-900">Two ways customers use the service today:</h4>
              <ul className="list-disc space-y-2 pl-5 text-sm text-slate-600">
                <li>
                  Capture a lab-owned environment so you can iterate and post-train policies against a space you control before
                  inviting external stakeholders.
                </li>
                <li>
                  Scan the exact warehouse, grocery, or retail floor you’re selling into, then simulate workflows to quantify
                  savings, prove uptime, and de-risk the rollout before robots ever arrive.
                </li>
              </ul>
            </div>
            <div className="grid gap-4 md:grid-cols-3">
              {["Scan", "Rebuild", "Prove"].map((step, index) => {
                ...
                return (
                  <div key={step} className="space-y-2 rounded-3xl border border-slate-200 p-5">
                    <span className="text-xs font-semibold uppercase tracking-[0.3em] text-slate-400">
                      {String(index + 1).padStart(2, "0")}
                    </span>
                    <h5 className="text-base font-semibold text-slate-900">{step}</h5>
          ...
          <div className="space-y-4 rounded-3xl border border-slate-200 bg-slate-50 p-5">
            <h4 className="text-sm font-semibold text-slate-900">Reserve your slot</h4>
            <p className="text-sm text-slate-600">
              Priority goes to facilities with active robotic deployments. Join the waitlist and we’ll coordinate capture
              windows, SLAs, and pricing.
            </p>
          </div>

          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-2">
              <label className="text-xs uppercase tracking-[0.3em] text-slate-400" htmlFor="scene-facility-address">
                Facility address
              </label>
              <input
                ref={addressInputRef}
              ...
              {placesUnavailable ? (
                <p className="text-xs text-slate-500">Autocomplete unavailable—enter the address manually.</p>
              ) : (
                <p className="text-xs text-slate-400">Powered by Google Places Autocomplete.</p>
              )}
            </div>
            <div className="space-y-2">
              <label className="text-xs uppercase tracking-[0.3em] text-slate-400" htmlFor="scene-location-type">
                Location type
              </label>
              <input
          ...
          <div className="space-y-2">
            <label className="text-xs uppercase tracking-[0.3em] text-slate-400" htmlFor="scene-notes">
              Capture context
            </label>
            <textarea
              id="scene-notes"
          ...
          <div className="space-y-2">
            <label className="text-xs uppercase tracking-[0.3em] text-slate-400" htmlFor="scene-simulator">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <div className="space-y-2">
            ...
            <select
              ...
              className="w-full rounded-full border border-slate-200 bg-slate-50 px-4 py-3 text-sm focus:border-slate-400 focus:outline-none"
            >
              {(requestType === "scene" ? sceneBudgetRanges : datasetBudgetRanges).map((range) => (
          ...
          <div className="space-y-2">
            <label className="text-xs uppercase tracking-[0.3em] text-slate-400">
              {requestType === "scene" ? "Preferred capture window" : "Deadline"}
            </label>
            {requestType === "scene" ? (
              <input
                type="text"
