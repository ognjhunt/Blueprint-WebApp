      ...
      fileAnchors.forEach((anchor) => {
        ...
        if (determinedFileType === "image") {
          ...
          img.onload = () => {
            ...
            try {
              let aspect;
              if (anchor.width && anchor.height && anchor.width > 0 && anchor.height > 0) {
                  aspect = anchor.width / anchor.height;
                  console.log(`[ThreeViewer fileAnchors Image] Using dimensions from anchor data for ${anchor.id}: ${anchor.width}x${anchor.height}, Aspect: ${aspect}`);
              } else {
                  aspect = img.width / img.height;
                  console.log(`[ThreeViewer fileAnchors Image] Using dimensions from loaded image for ${anchor.id}: ${img.width}x${img.height}, Aspect: ${aspect}`);
              }
          ...
          video.onloadeddata = () => {
            ...
            try {
              ...
              if (anchor.width && anchor.height && anchor.width > 0 && anchor.height > 0) {
                  aspect = anchor.width / anchor.height;
                  console.log(`[ThreeViewer fileAnchors Video] Using dimensions from anchor data for ${anchor.id}: ${anchor.width}x${anchor.height}, Aspect: ${aspect}`);
              } else {
                  aspect = video.videoWidth / video.videoHeight;
                  console.log(`[ThreeViewer fileAnchors Video] Using dimensions from loaded video for ${anchor.id}: ${video.videoWidth}x${video.videoHeight}, Aspect: ${aspect}`);
              }
