...
function MobileBenefitsCarousel({ benefits }) {
  ...
  const onTouchEnd = () => {
    ...
    if (dist > 50 && currentBenefit < benefits.length - 1) setCurrentBenefit(currentBenefit + 1);
  ...
  return (
    <div className="relative px-4">
      ...
      >
        <h2 className="text-2xl md:text-3xl font-black mb-3 text-white">Why Join the Pilot Program?</h2>
        <p className="text-sm text-slate-300">Experience the future of customer engagement with zero risk</p>
      </motion.div>

      <div className="text-center mb-4">
        <span className="text-sm font-medium text-slate-400">Benefit {currentBenefit + 1} of {benefits.length}</span>
      </div>
              ...
              <CardContent className="p-6 text-center">
                <div className="flex justify-center mb-6">
                  <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-emerald-200/20 to-cyan-200/20 flex items-center justify-center shadow-lg">
                    {React.cloneElement(current.icon, { className: "w-10 h-10 text-emerald-300" })}
                  </div>
                </div>

                <Badge className="bg-emerald-500/10 text-emerald-300 border-emerald-500/30 text-sm mb-4 px-4 py-2 font-bold">✨ {current.highlight}</Badge>

                <h3 className="text-2xl font-bold mb-4 text-white">{current.title}</h3>
                <p className="text-slate-300 leading-relaxed text-base mb-6">{current.description}</p>
                ...
                <div className="bg-gradient-to-r from-emerald-50/10 to-cyan-50/10 border border-emerald-500/20 rounded-xl p-4">
                  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
                    <h4 className="font-bold text-white mb-3 text-sm">What This Means For You:</h4>
                    <div className="grid grid-cols-1 gap-2 text-xs">
                      {getBenefitDetails(current.title).map((detail, idx) => (
                        <motion.div key={idx} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 + idx * 0.08 }} className="flex items-center gap-2">
      ...
      <div className="flex justify-between items-center mt-6">
        <Button variant="outline" size="sm" onClick={() => currentBenefit > 0 && setCurrentBenefit(currentBenefit - 1)} disabled={currentBenefit === 0} className="flex items-center gap-2 disabled:opacity-50 border-white/20 text-slate-200">
        ...
        <div className="flex gap-1">
          {benefits.map((_, idx) => (
            <button key={idx} onClick={() => setCurrentBenefit(idx)} className={`w-2 h-2 rounded-full transition-all duration-300 ${idx === currentBenefit ? "bg-emerald-500 w-4" : "bg-slate-500/40"}`} />
          ))}
        </div>

        <Button variant="outline" size="sm" onClick={() => currentBenefit < benefits.length - 1 && setCurrentBenefit(currentBenefit + 1)} disabled={currentBenefit === benefits.length - 1} className="flex items-center gap-2 disabled:opacity-50 border-white/20 text-slate-200">
          Next
      ...
      {currentBenefit === 0 && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1 }} className="text-center mt-4">
          <p className="text-xs text-slate-400">👆 Swipe left/right to see all benefits</p>
        </motion.div>
      )}

      <motion.div className="mt-8 bg-white/5 border border-white/10 rounded-xl p-4" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.3 }}>
        <h4 className="font-bold text-white mb-3 text-sm text-center">All Benefits at a Glance</h4>
        <div className="flex justify-center gap-3">
          {benefits.map((b, idx) => (
            <button key={idx} onClick={() => setCurrentBenefit(idx)} className={`p-3 rounded-xl transition-all ${idx === currentBenefit ? "bg-emerald-500 text-white shadow-lg scale-110" : "bg-white/10 text-slate-300 border border-white/10 hover:bg-white/15"}`} title={b.title}>
              {React.cloneElement(b.icon, { className: "w-5 h-5" })}
            </button>
          ))}
        </div>
      </motion.div>

      <motion.div className="mt-6 text-center" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.4 }}>
        <p className="text-sm text-slate-300 mb-3">Ready to experience the future of customer engagement?</p>
        <div className="flex flex-col gap-2">
          <Button className="rounded-xl bg-gradient-to-r from-emerald-500 to-cyan-600 text-white px-6 py-3 text-sm font-semibold" onClick={() => {
            const el = document.getElementById("pilot-waitlist");
            if (el) el.scrollIntoView({ behavior: "smooth" });
          }}>
...
function getBenefitDetails(title) {
  const details = {
    "Zero Risk": ["No upfront costs or commitments", "Test before you invest", "Full support throughout trial", "No hidden fees or surprises"],
    "Premium Hardware": ["Apple Vision Pro demos included", "Latest AR technology access", "Professional setup & training", "Hardware experts on-site"],
    "Real Analytics": ["Customer engagement metrics", "ROI measurement tools", "Actionable insights provided", "Performance optimization tips"],
    "Future-Ready": ["First-mover advantage", "Next-gen customer experiences", "Competitive differentiation", "Innovation leadership"],
  };
  return details[title] || ["Enhanced customer experience", "Business growth opportunities", "Technology advancement", "Competitive advantage"];
}
...
function MobileBusinessTypesCarousel({ idealFor }) {
  ...
  const onTouchEnd = () => {
    if (!touchStart || !touchEnd) return;
    const dist = touchStart - touchEnd;
    if (dist > 50 && currentType < idealFor.length - 1) setCurrentType(currentType + 1);
  ...
  return (
    <div className="relative px-4">
      <motion.div className="text-center mb-8" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
        <h2 className="text-2xl md:text-3xl font-black mb-3 text-white">Perfect For Forward-Thinking Businesses</h2>
        <p className="text-sm text-slate-300">If you have a physical space, Blueprint is for you</p>
      </motion.div>

      <div className="text-center mb-4">
        <span className="text-sm font-medium text-slate-400">{currentType + 1} of {idealFor.length} Business Types</span>
      </div>

      <div className="relative overflow-hidden" onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={onTouchEnd}>
        <AnimatePresence mode="wait">
          <motion.div key={currentType} initial={{ x: 300, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -300, opacity: 0 }} transition={{ type: "spring", stiffness: 300, damping: 30 }}>
            <Card className="bg-white/5 backdrop-blur border-white/10 shadow-xl">
              <CardContent className="p-6 text-center">
                <div className="flex justify-center mb-6">
                  <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-emerald-200/20 to-cyan-200/20 flex items-center justify-center shadow-lg">
                ...
                <div className="bg-gradient-to-r from-emerald-50/10 to-cyan-50/10 border border-emerald-500/20 rounded-xl p-4">
                  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
                    <h4 className="font-bold text-white mb-3 text-sm">Perfect For:</h4>
                    <div className="grid grid-cols-1 gap-2 text-xs">
      ...
      <div className="flex justify-between items-center mt-6">
        <Button variant="outline" size="sm" onClick={() => currentType > 0 && setCurrentType(currentType - 1)} disabled={currentType === 0} className="flex items-center gap-2 disabled:opacity-50 border-white/20 text-slate-200">
          <ChevronRight className="w-4 h-4 rotate-180" />
          Previous
        </Button>
        <div className="flex gap-1">
          {idealFor.map((_, idx) => (
            <button key={idx} onClick={() => setCurrentType(idx)} className={`w-2 h-2 rounded-full transition-all duration-300 ${idx === currentType ? "bg-emerald-500 w-4" : "bg-slate-500/40"}`} />
          ))}
        </div>
        <Button variant="outline" size="sm" onClick={() => currentType < idealFor.length - 1 && setCurrentType(currentType + 1)} disabled={currentType === idealFor.length - 1} className="flex items-center gap-2 disabled:opacity-50 border-white/20 text-slate-200">
          Next
          <ChevronRight className="w-4 h-4" />
        </Button>
      </div>
      ...
      {currentType === 0 && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1 }} className="text-center mt-4">
          <p className="text-xs text-slate-400">👆 Swipe left/right to see more business types</p>
        </motion.div>
      )}

      <motion.div className="mt-8 bg-white/5 border border-white/10 rounded-xl p-4" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.3 }}>
        <h4 className="font-bold text-white mb-3 text-sm text-center">Quick Access</h4>
        <div className="grid grid-cols-2 gap-2">
          {idealFor.map((biz, idx) => (
            <button key={idx} onClick={() => setCurrentType(idx)} className={`p-2 rounded-lg text-xs font-medium transition-all flex items-center gap-2 ${idx === currentType ? "bg-emerald-500 text-white" : "bg-white/10 text-slate-300 border border-white/10 hover:bg-white/15"}`}>
              {React.cloneElement(biz.icon, { className: "w-4 h-4 flex-shrink-0" })}
              <span className="truncate">{biz.title.split("&")[0].trim()}</span>
            </button>
          ))}
        </div>
      </motion.div>
      ...
      <motion.div className="mt-6 text-center" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.4 }}>
        <p className="text-sm text-slate-300 mb-3">Ready to transform your space?</p>
        <Button size="sm" className="bg-gradient-to-r from-emerald-500 to-cyan-600 text-white px-6 py-2 text-sm" onClick={() => { const el = document.getElementById("pilot-waitlist"); if (el) el.scrollIntoView({ behavior: "smooth" }); }}>
...
function getUseCaseExamples(type) {
  const ex = {
    "Retail & Grocery Stores": ["Product information on demand", "Virtual try-on experiences", "Interactive shopping lists", "Promotional offers"],
    "Museums & Galleries": ["Immersive historical tours", "Interactive art experiences", "Multi-language guides", "Educational content"],
    "Restaurants & Cafes": ["3D menu visualization", "Nutritional information", "Order customization", "Table service enhancement"],
    "Real Estate & Showrooms": ["Virtual property staging", "Product customization", "Space visualization", "Feature highlights"],
  };
  return ex[type] || ["Interactive experiences", "Customer engagement", "Information access", "Enhanced service"];
}
...
function MobileTimelineCarousel({ timeline }) {
  ...
  const onTouchEnd = () => {
    if (!touchStart || !touchEnd) return;
    const dist = touchStart - touchEnd;
    if (dist > 50 && currentStep < timeline.length - 1) setCurrentStep(currentStep + 1);
    ...
    <div className="relative">
      <div className="flex justify-center mb-8">
        <div className="flex items-center gap-2">
          {timeline.map((_, idx) => (
            <button key={idx} onClick={() => setCurrentStep(idx)} className={`h-2 rounded-full transition-all duration-300 ${idx === currentStep ? `w-8 bg-gradient-to-r ${step.color}` : idx < currentStep ? "w-2 bg-emerald-400" : "w-2 bg-slate-500/40"}`} />
          ))}
        </div>
      </div>

      <div className="text-center mb-6">
        <span className="text-sm font-medium text-slate-300">Step {currentStep + 1} of {timeline.length}</span>
      </div>

      <div className="relative overflow-hidden" onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={onTouchEnd}>
        <AnimatePresence mode="wait">
          <motion.div key={currentStep} initial={{ x: 300, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -300, opacity: 0 }} transition={{ type: "spring", stiffness: 300, damping: 30 }}>
            <Card className="bg-white/5 backdrop-blur border-white/10 shadow-xl">
              <CardContent className="p-6">
                <div className="flex justify-center mb-6">
                  <div className={`w-16 h-16 rounded-full bg-gradient-to-br ${step.color} flex items-center justify-center shadow-lg text-white`}>
                    {React.cloneElement(step.icon, { className: "w-7 h-7" })}
                  </div>
                </div>
                <div className="text-center mb-6">
                  <h3 className="text-2xl font-bold mb-2 text-white">{step.title}</h3>
                  <p className="text-emerald-300 font-semibold text-lg">{step.subtitle}</p>
                </div>
                <p className="text-slate-300 mb-6 text-center leading-relaxed">{step.description}</p>
                ...
                <div className="text-center">
                  <Badge className="bg-cyan-500/10 text-cyan-300 border-cyan-500/30 px-4 py-2 text-sm font-semibold">✨ {step.benefit}</Badge>
                </div>
              </CardContent>
            </Card>
          </motion.div>
      ...
      <div className="flex justify-between items-center mt-6">
        <Button variant="outline" size="sm" onClick={() => setCurrentStep((s) => Math.max(0, s - 1))} disabled={currentStep === 0} className="flex items-center gap-2 disabled:opacity-50 border-white/20 text-slate-200">
          <ChevronRight className="w-4 h-4 rotate-180" />
          Previous
        </Button>
        <div className="flex gap-1">
          {timeline.map((_, idx) => (
            <div key={idx} className={`w-2 h-2 rounded-full transition-colors ${idx === currentStep ? "bg-emerald-500" : "bg-slate-500/40"}`} />
          ))}
        </div>
        <Button variant="outline" size="sm" onClick={() => setCurrentStep((s) => Math.min(timeline.length - 1, s + 1))} disabled={currentStep === timeline.length - 1} className="flex items-center gap-2 disabled:opacity-50 border-white/20 text-slate-200">
          Next
          <ChevronRight className="w-4 h-4" />
        </Button>
      </div>
      ...
      {currentStep === 0 && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1 }} className="text-center mt-4">
          <p className="text-xs text-slate-400">👆 Swipe left/right or use buttons to navigate</p>
        </motion.div>
      )}
    </div>
  );
...
function MobileFAQCarousel({ faqs = [] }) {
  ...
  const onTouchEnd = () => {
    if (!touchStart || !touchEnd) return;
    const dist = touchStart - touchEnd;
    if (dist > 50 && currentFaq < faqs.length - 1) setCurrentFaq(currentFaq + 1);
  ...
  return (
    <div className="relative px-4">
      <motion.div className="text-center mb-6" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
        <h2 className="text-2xl md:text-3xl font-black mb-3 text-white">Frequently Asked Questions</h2>
        <p className="text-sm text-slate-300">Got questions? We’ve got answers.</p>
      </motion.div>

      <div className="text-center mb-4">
        <span className="text-sm font-medium text-slate-400">Question {currentFaq + 1} of {faqs.length}</span>
      </div>

      <div className="relative overflow-hidden" onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={onTouchEnd}>
        <AnimatePresence mode="wait">
          <motion.div key={currentFaq} initial={{ x: 300, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -300, opacity: 0 }} transition={{ type: "spring", stiffness: 300, damping: 30 }}>
              ...
              <CardContent className="p-6">
                <div className="mb-4">
                  <div className="flex items-start gap-3 mb-4">
                    ...
                    <h3 className="text-lg font-bold text-white leading-tight">{currentFaqData.q}</h3>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <motion.p initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="text-slate-300 leading-relaxed text-sm">
                      {currentFaqData.a}
                    </motion.p>
                  </div>
                </div>
      ...
      <div className="flex justify-between items-center mt-6">
        <Button variant="outline" size="sm" onClick={() => currentFaq > 0 && setCurrentFaq(currentFaq - 1)} disabled={currentFaq === 0} className="flex items-center gap-2 disabled:opacity-50 border-white/20 text-slate-200">
          <ChevronRight className="w-4 h-4 rotate-180" />
          Previous
        </Button>
        ...
        <div className="flex gap-1">
          {faqs.map((_, idx) => (
            <button key={idx} onClick={() => setCurrentFaq(idx)} className={`w-2 h-2 rounded-full transition-all duration-300 ${idx === currentFaq ? "bg-emerald-500 w-4" : "bg-slate-500/40"}`} />
          ))}
        </div>

        <Button variant="outline" size="sm" onClick={() => currentFaq < faqs.length - 1 && setCurrentFaq(currentFaq + 1)} disabled={currentFaq === faqs.length - 1} className="flex items-center gap-2 disabled:opacity-50 border-white/20 text-slate-200">
          Next
          <ChevronRight className="w-4 h-4" />
        </Button>
      </div>
      ...
      {currentFaq === 0 && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1 }} className="text-center mt-4">
          <p className="text-xs text-slate-400">👆 Swipe left/right to see more questions</p>
        </motion.div>
      )}

      <motion.div className="mt-8 bg-white/5 border border-white/10 rounded-xl p-4" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.3 }}>
        <h4 className="font-bold text-white mb-3 text-sm text-center">Popular Questions</h4>
        <div className="flex flex-wrap gap-2 justify-center">
          {[0, 1, 2, 3].map((idx) => (
            <button key={idx} onClick={() => setCurrentFaq(idx)} className={`px-3 py-1.5 rounded-full text-xs font-medium transition-all ${idx === currentFaq ? "bg-emerald-500 text-white" : "bg-white/10 text-slate-300 border border-white/10 hover:bg-white/15"}`}>
              Q{idx + 1}
            </button>
          ))}
        </div>
      ...
      <motion.div className="mt-6 text-center" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.4 }}>
        ...
        <div className="flex flex-col gap-2">
          <Button variant="outline" size="sm" className="border-white/30 text-white hover:bg-white/10 text-sm" asChild>
            <a href="mailto:nijel@tryblueprint.io">
              <Send className="mr-2 w-4 h-4" />
              Email Us
            </a>
          </Button>
          <Button size="sm" className="bg-gradient-to-r from-emerald-500 to-cyan-600 text-white hover:from-emerald-600 hover:to-cyan-700 text-sm" asChild>
...
export default function PilotProgram() {
    ...
    toast.custom((t) => (
      <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 p-6 max-w-md mx-auto">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 className="font-bold text-slate-900">Pilot Access Required</h3>
            <p className="text-sm text-slate-600">Check your email for invitation</p>
          </div>
        </div>
        <p className="text-slate-700 mb-4 text-sm leading-relaxed">Please use the unique URL we sent to your email to access the pilot program.</p>
        <div className="space-y-3">
          <p className="text-xs text-slate-500">Didn't receive a pilot invitation email?</p>
          <button onClick={() => { toast.dismiss(t); const el = document.getElementById("pilot-waitlist"); if (el) el.scrollIntoView({ behavior: "smooth" }); }} className="w-full bg-gradient-to-r from-emerald-500 to-cyan-600 text-white px-4 py-3 rounded-xl font-semibold text-sm transition-all hover:scale-[1.02]">
            Sign up for the Pilot Program
          </button>
          <button onClick={() => toast.dismiss(t)} className="w-full text-slate-500 hover:text-slate-700 px-4 py-2 text-sm font-medium">Close</button>
        </div>
      </div>
  ...
  const timeline = [
    {
      icon: <MapPin className="w-6 h-6" />, title: "Week 1", subtitle: "Mapping Session",
      description: "We scan your venue in 30-60 minutes using advanced LiDAR technology to create a precise digital twin.",
      color: "from-emerald-500 to-cyan-500", benefit: "Zero prep work required",
      details: ["Quick 30-60 min scan", "No business disruption", "Millimeter-accurate 3D model"],
    },
    {
      icon: <Sparkles className="w-6 h-6" />, title: "Week 1", subtitle: "Finalize Design",
      description: "Our AI helps our designers create custom AR experiences, and we work with you to finalize the design and smart triggers.",
      color: "from-cyan-500 to-teal-500", benefit: "Fully customized for your business",
      details: ["AI-generated content", "Brand-aligned design", "Interactive elements", "Final adjustments"],
    },
    {
      icon: <PlayCircle className="w-6 h-6" />, title: "Week 2", subtitle: "Demo Day",
      description: "Experience your space transformed with AR using cutting-edge devices. See the magic firsthand!",
      color: "from-emerald-400 to-cyan-400", benefit: "Hands-on experience",
      details: ["Vision Pro demo", "Team training", "Immediate feedback"],
    },
  ];

  const idealFor = [
    { icon: <Store className="w-8 h-8" />, title: "Retail & Grocery Stores", description: "Transform shopping with interactive product demos and virtual try-ons" },
    { icon: <Building2 className="w-8 h-8" />, title: "Museums & Galleries", description: "Bring exhibits to life with immersive storytelling and digital guides" },
    { icon: <UtensilsCrossed className="w-8 h-8" />, title: "Restaurants & Cafes", description: "Interactive menus, 3D food visualization, and tableside ordering experiences" },
    { icon: <Users className="w-8 h-8" />, title: "Real Estate & Showrooms", description: "Let customers visualize products in their space before buying" },
  ];

  const benefits = [
    { icon: <Shield className="w-6 h-6" />, title: "Zero Risk", description: "2 week program with absolutely no cost or obligation", highlight: "100% FREE" },
    { icon: <Monitor className="w-6 h-6" />, title: "Premium Hardware", description: "Test with Apple Vision Pro and other cutting-edge devices", highlight: "No purchase needed" },
    { icon: <Target className="w-6 h-6" />, title: "Real Analytics", description: "Track engagement, dwell time, and conversion metrics", highlight: "Data-driven insights" },
    { icon: <Zap className="w-6 h-6" />, title: "Future-Ready", description: "Be first to market with next-gen customer experiences", highlight: "Competitive edge" },
  ];

  const faqs = [
    { q: "What exactly is Blueprint?", a: "Blueprint is the easiest way to create custom augmented reality experiences for physical spaces. Using AI and spatial computing, we transform your venue into an interactive digital environment. We call these 'Blueprints'. Customers access Blueprints through their smart glasses—no app download required." },
    { q: "What does the pilot program cost?", a: "Nothing. The entire 2 week program is completely free with no hidden fees, no credit card required, and no obligation. This includes mapping, AI-generated AR content, hardware demos, and analytics." },
    { q: "How long does the space mapping take?", a: "Most spaces under 5,000 sq ft are mapped in 30–60 minutes. Larger venues may take 1–2 hours. The process is quiet, non-invasive, and won't disrupt operations." },
    { q: "Do we need special hardware for customers?", a: "No. Customers access the AR experience via the web—no app download. During the pilot, we bring Vision Pro and other devices for demos, but day-to-day usage just needs the customer's device." },
    { q: "What do you need from us?", a: "1) Access to your space for the initial mapping, 2) Basic information about your business/products/exhibits, and 3) Feedback. We handle the technology, content, and implementation." },
    { q: "What kinds of AR experiences can you create?", a: "Retail: virtual try-ons, product demos. Museums: digital guides, historical recreations. Restaurants: 3D menus and tableside ordering. Real estate: virtual staging and property tours." },
    { q: "What happens after the 2 week program?", a: "We'll send a brief survey to Demo Day participants. Your feedback helps us improve Blueprint." },
    { q: "How do customers access the AR experience?", a: "You'll get custom QR codes to place around your space. Customers scan and the AR experience launches instantly. No downloads—just magic." },
  ];
    ...
    <div className="min-h-screen flex flex-col relative overflow-hidden bg-[#0B1220] text-slate-100">
          ...
          <div className="flex flex-col lg:flex-row items-center gap-8 lg:gap-16">
            <FadeIn yOffset={20} delay={0.1} className="flex-1 text-center lg:text-left">
              ...
              <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-black mb-6 leading-tight text-white">
                ...
                <span className="bg-gradient-to-r from-emerald-300 via-cyan-300 to-emerald-200 bg-clip-text text-transparent">Into an Interactive AR Experience</span>
              </h1>

              <p className="text-base sm:text-lg md:text-xl text-slate-300 mb-6 md:mb-8 max-w-2xl mx-auto lg:mx-0">
                Blueprint uses AI to create custom augmented reality for your business. No apps needed — customers just scan and explore. Try it free.
              </p>

              <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start mb-8">
                <Button size="lg" className="rounded-xl bg-gradient-to-r from-emerald-500 to-cyan-600 text-white px-6 sm:px-8 py-6 text-base sm:text-lg font-bold shadow-xl transition-transform hover:scale-[1.02]" onClick={showPilotToast}>
                  Start Your Free 2 Week Program
                  <ArrowRight className="ml-2 w-5 h-5" />
                </Button>
                <Button size="lg" className="rounded-xl px-6 sm:px-8 py-6 text-base sm:text-lg bg-white/90 text-slate-900 border border-white/60 backdrop-blur hover:bg-white hover:border-white" onClick={handleWatchDemo}>
              ...
              <div className="flex flex-col sm:flex-row gap-2 md:gap-4 mb-2 text-xs md:text-sm">
                <div className="flex items-center gap-2"><CheckCircle2 className="w-5 h-5 text-emerald-400" /><span className="text-slate-200">Works on all smart glasses</span></div>
                <div className="flex items-center gap-2"><CheckCircle2 className="w-5 h-5 text-emerald-400" /><span className="text-slate-200">No app downloads</span></div>
                <div className="flex items-center gap-2"><CheckCircle2 className="w-5 h-5 text-emerald-400" /><span className="text-slate-200">AI‑generated content</span></div>
              </div>
            </FadeIn>
            ...
            <FadeIn yOffset={20} delay={0.2} className="flex-1 w-full max-w-2xl lg:max-w-none" id="demo-video">
              <div className="relative rounded-2xl overflow-hidden shadow-2xl">
                ...
                <video ref={videoRef} className="w-full aspect-video object-cover" src="/videos/blueprint-website-demo2.mp4" controls preload="metadata" />
              </div>
              <p className="text-center text-sm text-slate-400 mt-4">See Blueprint in action at an art museum</p>
            </FadeIn>
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-4 sm:px-6 max-w-7xl">
          ...
          <div className="hidden md:block">
            <FadeIn yOffset={20} delay={0.1} className="text-center mb-12">
              <h2 className="text-3xl md:text-4xl font-bold mb-4 text-white">Perfect For Forward‑Thinking Businesses</h2>
              <p className="text-lg text-slate-300 max-w-3xl mx-auto">If you have a physical space and want unforgettable customer experiences, Blueprint is for you.</p>
            </FadeIn>
                  ...
                  <Card className="h-full bg-white/90 border-white/10 hover:shadow-xl transition-all">
                    <CardContent className="p-4 md:p-6 text-center">
                      <div className="w-16 h-16 mx-auto mb-4 rounded-2xl bg-gradient-to-br from-emerald-200/30 to-cyan-200/30 flex items-center justify-center group-hover:scale-110 transition-transform">
                        {React.cloneElement(item.icon, { className: "w-8 h-8 text-emerald-600" })}
                      </div>
      ...
      <section className="container mx-auto px-4 sm:px-6 py-16 max-w-7xl">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            { label: "Investment Required", value: "$0", icon: <DollarSign className="w-6 h-6" />, description: "Completely free pilot program", gradient: "from-green-500 to-emerald-500" },
            { label: "Time to Launch", value: "< 2 weeks", icon: <Clock className="w-6 h-6" />, description: "From scan to live experience", gradient: "from-cyan-500 to-teal-500" },
            { label: "Setup Time", value: "30–60 mins", icon: <Zap className="w-6 h-6" />, description: "Quick venue scanning process", gradient: "from-emerald-400 to-cyan-400" },
          ].map((stat, idx) => (
                ...
                <CardContent className="p-6">
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="text-sm text-slate-600 mt-2">{stat.description}</p>
                    </div>
                    <div className={`p-3 rounded-xl bg-gradient-to-br ${stat.gradient} opacity-10 group-hover:opacity-20 transition-opacity`}>{React.cloneElement(stat.icon, { className: "w-6 h-6 text-slate-900" })}</div>
                  </div>
                </CardContent>
              </Card>
            </FadeIn>
      ...
      <section className="py-16 md:py-24 bg-[#0E172A]">
        <div className="container mx-auto px-4 sm:px-6 max-w-7xl">
          <FadeIn yOffset={20} delay={0.1} className="text-center mb-12 md:mb-20">
            <Badge className="inline-flex items-center gap-2 bg-cyan-500/10 text-cyan-300 border-cyan-500/30 mb-6 px-4 py-2"><CalendarCheck className="w-4 h-4" />Simple 2 Week Process</Badge>
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold mb-6 text-white">From First Scan to Live AR in Two Weeks</h2>
            <p className="text-lg md:text-xl text-slate-300 max-w-3xl mx-auto leading-relaxed">Our streamlined process gets your AR experience up and running fast, with zero technical work on your end.</p>
          </FadeIn>
          ...
          <div className="hidden md:block relative max-w-6xl mx-auto">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-12 relative">
              {timeline.map((step, idx) => (
                <FadeIn key={idx} yOffset={30} delay={idx * 0.15} className="relative">
                  <div className="flex justify-center mb-8 lg:mb-0">
                    <div className="lg:absolute lg:-top-[10px] lg:z-20">
                      <div className={`w-16 h-16 lg:w-14 lg:h-14 rounded-full bg-gradient-to-br ${step.color} flex items-center justify-center shadow-xl relative text-white border-4 border-white/90`}>
                        {React.cloneElement(step.icon, { className: "w-7 h-7 lg:w-6 lg:h-6" })}
                      </div>
                    </div>
                  </div>
                  ...
                  <Card className="h-full bg-white/95 border-white/10 hover:shadow-2xl transition-all duration-300 lg:mt-20 lg:pt-8">
                    <CardContent className="p-6 lg:p-8">
                      <div className="text-center mb-6">
                        <h3 className="text-xl lg:text-2xl font-bold mb-2 text-slate-900">{step.title}</h3>
                        <p className="text-emerald-600 font-semibold text-base lg:text-lg">{step.subtitle}</p>
                      </div>
                      <p className="text-slate-600 mb-6 text-center text-sm lg:text-base leading-relaxed">{step.description}</p>
                      <div className="space-y-3 mb-6">
                        {step.details.map((d, i) => (
                          <div key={i} className="flex items-start gap-3 text-sm lg:text-base"><CheckCircle2 className="w-4 h-4 lg:w-5 lg:h-5 text-emerald-600 flex-shrink-0 mt-0.5" /><span className="text-slate-700 leading-relaxed">{d}</span></div>
                        ))}
                      </div>
                      <div className="pt-4 border-t border-slate-200 text-center">
                        <Badge className="bg-cyan-500/10 text-cyan-700 border-cyan-200 px-3 py-1.5 text-sm font-semibold">✨ {step.benefit}</Badge>
                      </div>
                    </CardContent>
                  </Card>
                </FadeIn>
      ...
      <section className="py-16 md:py-24 relative bg-[#0B1220]">
        <div className="container mx-auto px-4 sm:px-6 max-w-7xl">
          <FadeIn yOffset={20} delay={0.1} className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-bold mb-4 text-white">Everything You Need to Know</h2>
            <p className="text-lg text-slate-300">Dive deeper into how Blueprint transforms your business</p>
          </FadeIn>
          ...
          <Tabs defaultValue="what" value={activeTab} onValueChange={setActiveTab} className="w-full">
                          ...
                          <AnimatePresence mode="wait">
                            ...
                            <TabsContent value="what" className="mt-0">
                              ...
                              >
                                <div>
                                  <h3 className="text-2xl font-bold mb-4 text-white">AI-Powered AR That Just Works</h3>
                                  <p className="text-slate-300 mb-6">
                                    Blueprint creates a smart digital layer over your physical space. Customers look at
                                    products, exhibits, or areas to unlock:
                                  </p>
                                  <ul className="space-y-4">
                                    {[
                                      ...
                                      <FadeIn key={idx} Component="li" delay={idx * 0.08} yOffset={8} className="flex items-start gap-3">
                                    ...
                                    <CardContent className="p-0">
                                      <div className="p-6">
                                        ...
                                        <p className="text-slate-300 text-sm">
                                          Customers scan a QR code and the AR experience launches in their browser. No
                            ...
                            <TabsContent value="why" className="mt-0">
                              <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -18 }} transition={{ duration: 0.28 }}>
                                <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
                                  <div>
                                    <h3 className="text-2xl font-bold mb-6 text-white">Proven Business Impact</h3>
                                    <p className="text-slate-300 mb-8">
                                      AR isn’t just cool tech — it drives real results. Pilot participants see immediate,
                                      measurable improvements:
                                    </p>

                                    <div className="grid grid-cols-2 gap-4 mb-8">
                                      {[
                                        { metric: "40%", label: "Increase in dwell time", icon: Clock },
                                        { metric: "3.5x", label: "More social shares", icon: Users },
                                        { metric: "28%", label: "Higher conversion", icon: Target },
                                        { metric: "92%", label: "Customer satisfaction", icon: Star },
                                      ].map((stat, idx) => (
                                        <FadeIn
                                          key={idx}
                                          delay={idx * 0.08}
                                  ...
                                  <div className="lg:pl-6">
                                    <div className="bg-white/5 border border-white/10 rounded-xl p-6 h-full flex flex-col justify-center">
                                      <h4 className="font-bold mb-4 text-white">Why AR Works So Well:</h4>
                                      <ul className="space-y-2 text-sm text-slate-300">
                                        ...
                                        <li>• Differentiates your brand in-store</li>
                                      </ul>
                                    </div>
                                  </div>
                                </div>
                            ...
                            <TabsContent value="how" className="mt-0">
                              ...
                              >
                                <h3 className="text-2xl font-bold mb-8 text-center text-white">The Technology Behind the Magic</h3>
                                  ...
                                  {[
                                    {
                                      ...
                                      description:
                                        "We use professional-grade LiDAR scanners to create a millimeter-accurate 3D model of your space. This digital twin is the foundation for your AR.",
                                    ...
                                    },
                                    {
                                      ...
                                      description:
                                        "The AR runs directly in modern browsers using WebXR, so customers get instant access without app downloads.",
                                    ...
                                    },
                                    {
                                      ...
                                      description:
                                        "Track engagement, interactions, and conversions as they happen. See what captures attention and iterate.",
                                      tech: "Custom Analytics Platform",
                                    },
                                  ].map((item, idx) => (
                                    <FadeIn key={idx} delay={idx * 0.08} yOffset={8} className="flex gap-6">
                    ...
                    <section id="benefits-section" className="py-16 md:py-24 bg-[#0E172A]">
                      <div className="container mx-auto px-4 sm:px-6 max-w-7xl">
                        ...
                        <div className="hidden md:block">
                          <FadeIn yOffset={20} delay={0.1} className="text-center mb-12">
                            <h2 className="text-3xl md:text-4xl font-bold mb-4 text-white">Why Join the Pilot Program?</h2>
                            <p className="text-lg text-slate-300 max-w-3xl mx-auto">
                              Experience the future of customer engagement with zero risk and maximum support
                            </p>
                          </FadeIn>
                                ...
                                <Card className="h-full bg-white/5 border-white/10 hover:shadow-2xl transition-all group">
                                  <CardContent className="p-4 md:p-6">
                                    <div className="w-10 h-10 md:w-14 md:h-14 rounded-xl md:rounded-2xl bg-gradient-to-br from-emerald-200/30 to-cyan-200/30 flex items-center justify-center mb-3 md:mb-4 group-hover:scale-110 transition-transform">
                                      {React.cloneElement(b.icon, { className: "w-5 h-5 md:w-6 md:h-6 text-emerald-300" })}
                                    </div>
                                    <Badge className="bg-emerald-500/10 text-emerald-300 border-emerald-500/30 text-xs mb-2 md:mb-3">
                                      {b.highlight}
                                    </Badge>
                                    <h3 className="text-lg md:text-xl font-bold mb-2 text-white">{b.title}</h3>
                                    <p className="text-slate-300 text-sm md:text-base">{b.description}</p>
                                  </CardContent>
                                </Card>
                              </FadeIn>
                            ))}
                      ...
                      <div className="container mx-auto px-4 sm:px-6 max-w-4xl">
                        ...
                        <div className="hidden md:block">
                          <FadeIn yOffset={20} delay={0.1} className="text-center mb-12">
                            ...
                            <p className="text-lg text-slate-300">Got questions? We’ve got answers.</p>
                          </FadeIn>
                              ...
                              <FadeIn key={idx} yOffset={14} delay={idx * 0.04}>
                                ...
                                >
                                  <div className="flex justify-between items-start gap-4">
                                    <h3 className="font-semibold text-lg pr-8 text-white">{faq.q}</h3>
                                    <ChevronDown
                          ...
                          >
                            ...
                            <p className="text-slate-300 mb-6">
                              Our team can help you understand how Blueprint can transform your business.
                            </p>
                            <div className="flex flex-col sm:flex-row gap-4 justify-center">
                              ...
                              <Button className="rounded-xl bg-gradient-to-r from-emerald-500 to-cyan-600 text-white" asChild>
                                <a href="https://calendly.com/blueprintar/30min">
                                  Schedule a Call
                                  <ArrowRight className="ml-2 w-5 h-5" />
                                </a>
                    ...
                    <section id="pilot-waitlist" className="bg-gradient-to-b from-white to-slate-50">
                      <ContactForm />
                    </section>
