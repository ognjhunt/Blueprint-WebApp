...
import { PILOT_FAQ, SUPPORT } from "@/components/pilotCopy";
...
const faqItems = PILOT_FAQ.map((f) => ({
  question: f.q,
  answer: f.a,
}));
...
export default function Help() {
  ...
  const [activeTab, setActiveTab] = useState<"reschedule" | "faq" | "contact">(
    "reschedule",
  );
  ...
  // Sync tab <-> URL hash (#reschedule/#faq/#contact)
  useEffect(() => {
    const initial = window.location.hash?.replace("#", "") as
      | "reschedule"
      | "faq"
      | "contact"
      | undefined;
  ...
  // Parses "3:30 PM" to a Date on the currently selected date
  const toDateAtTime = (date: Date | undefined, time: string) => {
    if (!date) return null;
    const [hhmm, ampm] = time.split(" ");
    const [h, m] = hhmm.split(":").map(Number);
    let hours = (h % 12) + (ampm.toUpperCase() === "PM" ? 12 : 0);
    const d = new Date(date);
    d.setHours(hours, m, 0, 0);
    return d;
  };

  const isPastTime = (date: Date | undefined, time: string) => {
    const dt = toDateAtTime(date, time);
    if (!dt) return false;
    const now = new Date();
    const isSameDay =
      date &&
    ...
    return isSameDay && dt.getTime() <= now.getTime();
  };
          ...
          >
            ...
            >
              <TabsList className="grid w-full grid-cols-3 mb-8 bg-white/5 p-1 rounded-lg">
                <TabsTrigger
                  value="reschedule"
                  className="inline-flex items-center gap-2 text-base text-slate-300 data-[state=active]:bg-gradient-to-r data-[state=active]:from-emerald-500 data-[state=active]:to-cyan-600 data-[state=active]:text-white rounded-md"
                >
                              ...
                              <FormItem>
                                ...
                                <FormControl>
                                  <div className="grid grid-cols-3 gap-2 mt-2">
                                    {timeSlots.map((time) => {
                                      const disabled = isPastTime(
                                        rescheduleForm.getValues("date"),
                                        time,
                                      );
                                      return (
                                        <div key={time}>
                                          ...
                                          <label
                                            ...
                                            className={`flex items-center justify-center px-3 py-2 text-sm rounded-md transition-all duration-200 ${
                                              disabled
                                                ...
                                                : "border border-slate-700 text-slate-300 cursor-pointer hover:bg-slate-800 peer-checked:bg-emerald-500/20 peer-checked:border-emerald-500 peer-checked:text-emerald-300"
                                            }`}
                                          >
                                            {time}
                                          </label>
                        ...
                        <div className="space-y-6">
                          ...
                          <div className="border-t border-white/10 pt-4 space-y-2">
                            <p className="text-sm text-slate-300">
                              {SUPPORT.reschedulePolicy}
                            </p>
                            <p className="text-xs text-slate-400">
                              {SUPPORT.mappingTimeNote}
                            </p>
                            <p className="text-xs text-slate-400">
                              {SUPPORT.demoWindowNote}
                            </p>
                            <Button
                              type="submit"
                              className="w-full bg-gradient-to-r from-emerald-500 to-cyan-600 text-white border-0 hover:from-emerald-400 hover:to-cyan-500"
              ...
              <TabsContent value="contact">
                ...
                >
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="space-y-6">
                      <Card className="bg-white/5 backdrop-blur border border-white/10 text-white">
                        <CardHeader>
