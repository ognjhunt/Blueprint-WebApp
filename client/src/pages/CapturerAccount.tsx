...
export default function CapturerAccount() {
  ...
  return (
    <main className="min-h-screen bg-runway-deep px-5 py-10 text-runway-text md:px-8">
      <div className="mx-auto flex max-w-[76rem] flex-col gap-7">
        <header className="border-b border-runway-line pb-8">
          <Eyebrow tone="brass" rule>Capture account</Eyebrow>
          <div className="mt-5 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <h1 className="font-display text-4xl font-semibold uppercase tracking-[0.005em] text-runway-text">Your captured-work record</h1>
        ...
        {query.isLoading ? <p className="py-10 text-runway-mute">Loading account records…</p> : null}
        ...
        {query.data ? (
          <>
            <section className="grid gap-4 sm:grid-cols-3">
              <Card pad="lg"><Banknote className="h-5 w-5 text-runway-signal" /><p className="mt-4 runway-meta">Lifetime paid</p><p className="runway-num mt-2 text-3xl text-runway-text">{money(query.data.earnings.total_earned_cents)}</p></Card>
              <Card pad="lg"><Clock3 className="h-5 w-5 text-runway-signal" /><p className="mt-4 runway-meta">Pending payout</p><p className="runway-num mt-2 text-3xl text-runway-text">{money(query.data.earnings.pending_payout_cents)}</p></Card>
              <Card pad="lg"><Camera className="h-5 w-5 text-runway-signal" /><p className="mt-4 runway-meta">Reviewed captures</p><p className="runway-num mt-2 text-3xl text-runway-text">{query.data.earnings.scans_completed}</p></Card>
            </section>

            <Card pad="lg" className="flex flex-col gap-5">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div><h2 className="font-display text-xl font-semibold uppercase tracking-[0.005em] text-runway-text">Payout setup</h2><p className="mt-2 text-sm text-runway-mute">Stripe owns bank-account and payout readiness.</p></div>
              ...
              {!isApproved ? <p className="text-sm text-runway-mute">Payout setup opens after application approval.</p> : null}
            ...
            <section className="flex flex-col gap-3">
              <h2 className="font-display text-xl font-semibold uppercase tracking-[0.005em] text-runway-text">Capture history</h2>
              {query.data.captures.length ? query.data.captures.map((capture) => (
                <Card key={capture.id} pad="md" className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <div><p className="font-semibold text-runway-text">{capture.target_address}</p><p className="runway-num mt-1 text-xs text-runway-faint">{capture.id} · {capture.captured_at ? new Date(capture.captured_at).toLocaleString() : "Time not recorded"}</p></div>
                  <StatusChip tone={statusTone(capture.status)} square>{capture.status?.replace(/_/g, " ") || "Not recorded"}</StatusChip>
                </Card>
              )) : <Card pad="lg"><p className="text-runway-mute">No capture submission is recorded for this account.</p></Card>}
            </section>

            <section className="flex flex-col gap-3">
              <h2 className="font-display text-xl font-semibold uppercase tracking-[0.005em] text-runway-text">Payout history</h2>
              {query.data.payouts.length ? query.data.payouts.map((payout) => (
                <Card key={payout.id} pad="md" className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <p className="font-semibold text-runway-text">{payout.description}</p>
                    <p className="runway-num mt-1 text-xs text-runway-faint">
                      {payout.id} · {new Date(payout.scheduled_for).toLocaleString()}
                    </p>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="runway-num text-sm text-runway-text">{money(payout.amount_cents)}</span>
                    <StatusChip tone={statusTone(payout.status)} square>{payout.status.replace(/_/g, " ")}</StatusChip>
                  </div>
                </Card>
              )) : <Card pad="lg"><p className="text-runway-mute">No payout ledger entry is recorded for this account.</p></Card>}
            </section>
