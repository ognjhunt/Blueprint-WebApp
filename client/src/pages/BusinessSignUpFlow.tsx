    ...
    <AuthLayout>
      ...
      <form className="auth-form auth-simple-signup" method="post" onSubmit={submit} noValidate aria-label={step === 1 ? "Account details" : "Workspace details"} aria-busy={busy}>
        {step === 1 ? <>
          ...
          <p className="auth-signup-note">{workspaceType === "site_operator" ? "Next, describe one job and film the work area. Your task page follows the assessment." : workspaceType === "robot_team" ? "Next, the task library. Site tasks are open to robot teams approved for early access." : "Next, start with a task or the task library."}</p>
