...
- Growth should stay anchored to one narrow commercial wedge at a time. The current priority wedge is **Site-Specific Robot Deployment Readiness**: one real site, one robot task, one threshold set, one capture-backed site package, one readiness report, and one pilot protocol or proof blocker, with explicit automatic policy enforcement on pricing, policy, rights, hosted access, and irreversible commitments.
