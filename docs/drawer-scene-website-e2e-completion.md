...
| 13. GPU policy episodes | **Attempted, not scored** — the release-bound run `team-eval-85e6d1ff3e66a639b2e8acf4d8a7737f-policy-canary-aef8b8388481` rented Vast instance 52622798 and passed bootstrap. The native reset gate found both desk occlusion in the overview view and a near-black wrist image whose exact readback showed the robot occupied 72.09% of pixels while the task occupied 22.26%. The worker made zero policy queries; its blocked cell placeholders are not scored episodes. |
| 14. Website result and teardown | **Open** — Vast teardown, a global provider-zero receipt, and an official $0.346 charge are proven for the failed activation. Retained website result delivery remains pending with `policy_canary_website_notification_readback_missing`; no policy result is visible on the task page. |
