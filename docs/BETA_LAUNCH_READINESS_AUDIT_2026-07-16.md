...
| BLR-005 | P1 | Two competing deploy paths possible; `render.yaml` had `autoDeploy: true` (any main push ships, red or green) | **follow-up in progress** | `.github/workflows/deploy.yml` deploys only the exact `workflow_run.head_sha` of a green CI run on main and now uses `RENDER_API_KEY` plus `RENDER_SERVICE_ID`; the duplicate in-CI hook job is removed. Keep dashboard auto-deploy on only until the API workflow proves one exact-SHA deployment, then disable it and verify the provider reports `autoDeploy: no`. Deployed SHA remains observable at `GET /version.json`; rollback stays revert-based. | inspect `render.yaml`, `.github/workflows/deploy.yml`; `curl <site>/version.json` post-deploy; retrieve Render service configuration |
...
| BLR-033 | Deploy activation: `RENDER_API_KEY` and `RENDER_SERVICE_ID` are configured; the API workflow must complete one hosted exact-SHA deployment before provider auto-deploy is disabled | Owner: webapp-codex for workflow proof, then blueprint-cto for provider-state verification. Steps in `docs/runbooks/ci-gated-deploy-2026-07-16.md` |
