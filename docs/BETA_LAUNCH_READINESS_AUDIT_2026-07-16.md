...
| BLR-011 | P2 | Residual prod-scope highs: `react-simple-maps`→`d3-color` ReDoS chain (only non-major fix is a downgrade to 1.0.0) | **deferred — accepted exception** | Package: react-simple-maps 3.x / d3-color <3.1. Advisory: GHSA d3-color ReDoS. Exploitability: low — client-side color parsing of repo-authored map styles, no untrusted input reaches `d3-color`. Owner: webapp lane. Expiry/retry: revisit when react-simple-maps ships a d3≥3.1 release or the map is replaced | `npm audit --omit=dev` shows only this chain at high |
