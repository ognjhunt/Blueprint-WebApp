...
| BLR-005 | P1 | Two competing deploy paths possible; `render.yaml` had `autoDeploy: true` (any main push ships, red or green) | **fixed and activated** | `.github/workflows/deploy.yml` deploys only the exact `workflow_run.head_sha` of a green CI run on main using `RENDER_API_KEY` plus `RENDER_SERVICE_ID`; the duplicate in-CI hook job is removed. Main CI run `29836994895` passed and gated run `29837245844` proved provider deploy `dep-d9fno0hkh4rs73cr9fsg` live at the exact SHA with both health endpoints at 200. Provider configuration was read back as `autoDeploy: no`. | inspect `render.yaml`, `.github/workflows/deploy.yml`; inspect run `29837245844` evidence; retrieve Render service configuration |
...
| BLR-033 | **Resolved 2026-07-21:** API workflow completed an exact-SHA deployment and provider auto-deploy was disabled | Closed by main CI run `29836994895`, gated deploy run `29837245844`, and provider readback `autoDeploy: no`. |
