...
- `docs/architecture/autonomous-scene-intake.md`: authenticated source/task consent, account authority, durable signed scene intent outbox, and Pipeline-owned status/revocation boundaries.
...
- Result-consumer truth starts in `docs/architecture/task-evaluation-result-delivery.md`, `server/routes/task-evaluation-results.ts`, `server/routes/evaluation-ready-runs.ts`, and `client/src/components/blueprint/app/PolicyCanaryResultPortal.tsx`. Registry admission, media-byte integrity, deterministic outcomes, controls, and email receipts are distinct facts. The result notification retry route is an explicit email mutation and is never invoked by status polling.
