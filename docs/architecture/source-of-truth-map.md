...
- Real-site robot evaluation dataset display truth starts in Pipeline artifacts under `pipeline/robot_eval_dataset/`, including `robot_team_test_submission_modalities.json`, then WebApp's advisory-only sync fields in `server/types/inbound-request.ts`, `client/src/types/inbound-request.ts`, `server/utils/pipelineAttachmentContract.ts`, `server/utils/pipelineStateMachine.ts`, and `server/utils/site-task-deployment-confidence.ts`.
