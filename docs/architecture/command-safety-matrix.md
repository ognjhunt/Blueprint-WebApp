...
`npm run autonomy:outcomes:snapshot`, `npm run autonomy:budget:recommend`, and `npm run autonomy:budget:dynamic:verify` are the safe local dynamic allocation loop. They implement `observe -> outcome snapshot -> score -> recommend -> human approval packet -> approved repo-local diff -> live system handled separately`.
