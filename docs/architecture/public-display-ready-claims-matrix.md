...
| This robot is ranked in generated-world policy evaluation, off-scope validated, collision/contact/manipulation validated, or guaranteed to hit thresholds | Request-scoped simulator traces, action logs, accepted anchor rows, rights/privacy proof, hosted/runtime proof, and buyer-approved methodology. | Use readiness advisory language and name the missing proof. |
| A policy API, container, trace, teleop demo, or sim-controller plugin passed a Blueprint evaluation | Hosted-session run records, owner-system logs, prediction/outcome ledger entries, and buyer-approved methodology tied to the exact site/task/scenario. | Say `reference submitted for review`, `missing proof tracked`, or `hosted-session run required`. |
