...
export interface NitrosendConfig {
  apiToken: string;
  baseUrl: string;
}

export interface NitrosendAudience {
  id: string;
  name: string;
  status: string;
  description?: string;
...
export interface NitrosendSequenceDraft {
  id: string;
  name: string;
  status: string;
  audienceId?: string;
...
export interface NitrosendCampaignDraft {
  id: string;
  name: string;
  status: string;
  audienceId?: string;
...
function nitrosendHeaders(config: NitrosendConfig) {
  return {
...
function normalizeNitrosendAudience(value: unknown): NitrosendAudience | null {
  ...
  return {
    ...
    estimatedSize: asNumber(record.estimatedSize),
  };
}

function normalizeNitrosendSequenceDraft(value: unknown): NitrosendSequenceDraft | null {
  const record = asRecord(value);
  if (!record) return null;
  const id = asString(record.id);
  const name = asString(record.name);
  ...
  return {
    ...
    name,
    status: asString(record.status) ?? "draft",
    audienceId: asString(record.audienceId),
    stepCount: asArray(record.steps).length || asNumber(record.stepCount) || 0,
  };
}

function normalizeNitrosendCampaignDraft(value: unknown): NitrosendCampaignDraft | null {
  const record = asRecord(value);
  if (!record) return null;
  const id = asString(record.id);
  const name = asString(record.name);
  ...
  return {
    ...
    status: asString(record.status) ?? "draft",
    audienceId: asString(record.audienceId),
    sequenceId: asString(record.sequenceId),
  };
}

export async function listNitrosendAudiences(
  config: NitrosendConfig,
  fetchImpl: FetchLike = fetch,
) {
  const payload = await fetchJson(fetchImpl, normalizeUrl(config.baseUrl, "/audiences"), {
    method: "GET",
    headers: nitrosendHeaders(config),
  });
  const items = asArray(asRecord(payload)?.items ?? payload)
    .map(normalizeNitrosendAudience)
    .filter((entry): entry is NitrosendAudience => Boolean(entry));
  return { audiences: items, count: items.length };
}

export async function upsertNitrosendAudience(
  config: NitrosendConfig,
  params: Record<string, unknown>,
  fetchImpl: FetchLike = fetch,
) {
  assertNitrosendDraftOnly(params);
  const payload = await fetchJson(fetchImpl, normalizeUrl(config.baseUrl, "/audiences"), {
    method: "POST",
    headers: nitrosendHeaders(config),
    body: JSON.stringify({
  ...
  const audience = normalizeNitrosendAudience(asRecord(payload)?.item ?? payload);
  if (!audience) {
    throw new Error("Nitrosend audience response did not include a valid draft audience.");
  }
  return audience;
}

export async function createNitrosendSequenceDraft(
  config: NitrosendConfig,
  params: Record<string, unknown>,
  fetchImpl: FetchLike = fetch,
) {
  assertNitrosendDraftOnly(params);
  const payload = await fetchJson(fetchImpl, normalizeUrl(config.baseUrl, "/sequences/drafts"), {
    method: "POST",
    headers: nitrosendHeaders(config),
    body: JSON.stringify({
      ...params,
  ...
  const sequence = normalizeNitrosendSequenceDraft(asRecord(payload)?.item ?? payload);
  if (!sequence) {
    throw new Error("Nitrosend sequence response did not include a valid draft sequence.");
  }
  return sequence;
}

export async function createNitrosendCampaignDraft(
  config: NitrosendConfig,
  params: Record<string, unknown>,
  fetchImpl: FetchLike = fetch,
) {
  assertNitrosendDraftOnly(params);
  const payload = await fetchJson(fetchImpl, normalizeUrl(config.baseUrl, "/campaigns/drafts"), {
    method: "POST",
    headers: nitrosendHeaders(config),
    body: JSON.stringify({
      ...params,
  ...
  const campaign = normalizeNitrosendCampaignDraft(asRecord(payload)?.item ?? payload);
  if (!campaign) {
