...
describe("execution dispatch verification", () => {
  ...
  it("accepts an execution lock only when fresh runtime evidence matches the expected assignee", () => {
    expect(
      verifyDispatchWake({
        wakeRunId: null,
        expectedAssigneeKey: "webapp-codex",
  ...
  it("rejects a stale issue execution lock without fresh runtime evidence", () => {
    expect(
      verifyDispatchWake({
        wakeRunId: null,
        expectedAssigneeKey: "webapp-codex",
