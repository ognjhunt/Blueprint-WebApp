...
- Durham: keep the lane centered on the highest-priority warehouse or facility candidates already named in the Durham launch packet, with owner/operator/tenant contacts, approval sequence, and draft-only outreach prepared before any live operator contact. Durham is activation-ready but still in refresh, so the operator lane should remain secondary to the buyer motion and should not imply that operator approval is universally required for lawful capture.
...
- `city-demand-agent`: mirror the Austin, San Francisco, San Diego, Durham, and Sacramento signals into city plans by marking which site clusters are operator-secondary versus operator-relevant, and which ones still need automatic policy block before any commercialization language is used.
- `site-operator-partnership-agent`: keep the Durham operator-lane packet focused on owner/operator/tenant contacts, approval sequence, and the first draft-only outreach note before any live operator contact is allowed.
