...
- done when:
  - The Seattle target ledger names the first proof candidates, queued lawful-access buckets, and longer-horizon discovery lanes.
...
- policy guardrail: Automatic policy block only if the policy expands spend, public posture, or channel scope beyond the bounded Seattle pilot.
- dependencies: none
...
- inputs:
  ...
  - autonomous Seattle launch posture
- done when:
...
- purpose: Give Intake, Field Ops, QA, and Rights lanes explicit Seattle rules so they can run without manual review.
...
- policy guardrail: Automatic policy block before any public posting or channel expansion beyond the written Seattle source policy.
...
- done when:
  - A curated first-wave Seattle prospect set is named with source bucket, rationale, lawful access posture, and next move.
...
- human owner: ops-lead
- purpose: Classify Seattle applicants using the approved rubric instead of ad hoc manual review.
...
- done when:
  - At least one Seattle proof-ready listing or equivalent proof pack exists with exact-site versus adjacent-site labeling.
...
- done when:
  - A named Seattle buyer target set is researched with facility/workflow fit and proof-path notes.
...
- done when:
  - Seattle buyer conversations are active with explicit next steps.
...
- done when:
  ...
  - A Work Queue breadcrumb exists for the current Seattle activation state and next policy block.
