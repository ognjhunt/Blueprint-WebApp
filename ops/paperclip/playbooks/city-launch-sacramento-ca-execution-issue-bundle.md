...
- human owner: growth-lead
- purpose: Keep Sacramento sourcing narrow, truthful, and fully autonomous inside written policy while explicitly distinguishing private controlled interiors from public, non-controlled commercial capture.
- policy_guardrail: Automatic policy block only if the plan expands spend, public posture, or channel scope beyond the bounded Sacramento pilot.
- dependencies: none
...
- inputs:
  ...
  - Sacramento autonomous launch posture
- done when:
...
- policy_guardrail: Automatic policy block only for unsupported rights/privacy handling or posture-changing source-policy changes beyond the approved Sacramento launch posture.
...
## Run Sacramento indoor public-location supply sourcing
...
- inputs:
  ...
  - city-launch-sacramento-ca-deep-research.md
  - ops/paperclip/playbooks/city-launch-sacramento-ca.md
  - Sacramento source policy
  - Sacramento city-opening brief
...
- done when:
  ...
  - Sacramento rejected candidates are saved with reason codes and evidence instead of silently disappearing.
  - Sacramento evidence log records the search queries and fetched URLs used after reusing the city Deep Research artifact as a source map.
  - Sacramento review candidates are validated with the seed script, materialized into a durable candidate ledger or app-visible review feed, and automatically passed through deterministic public-space review for the newly written ids without marking them rights-cleared, operator-approved, payable, or derived-world-model-ready.
...
- done when:
  ...
  - A Work Queue breadcrumb exists for the current Sacramento activation state and next policy block.
