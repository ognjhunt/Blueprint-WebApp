# Durham Intake Rubric, Trust Kit, and First-Capture Thresholds

## Scope
This playbook defines the Durham-specific operating packet for Intake, Field Ops, QA, and Rights.
...
Use this packet when a Durham contributor, applicant, or first-capture path needs a clear approval posture without dragging manual review into routine work.
...
## Durham Intake Stages

### 1. `sourced`
Use when Blueprint has identified a Durham prospect, but there is no usable live application yet.
...
Use when intake has asked for the minimum evidence set needed to evaluate Durham readiness.
...
### 5. `approved`
Use when the contributor is eligible for Durham cohort participation and can be onboarded into real capture work under current policy.
...
Use when Durham has a truthful first assignment and the capturer has enough context to show up or submit without improvising permission claims.
...
### Source Quality
Accept a Durham prospect only when the source is named and traceable enough to route real work.

Check:
- the source channel is one of the approved Durham channels or a named referral path
- the source bucket is explicit in the record
- the record does not invent provenance or overstate reach

### Access-Path Truth
Accept a Durham prospect only when the access path is honest about what is actually known.
...
Accept a Durham prospect only when the device and gear claim fit the lane.
...
Accept a Durham prospect only when the minimum packet is complete enough to trust the next routing step.
...
Use these thresholds as the Durham minimum progression rules:
...
When a Durham first assignment needs a site-facing explanation, the capturer may say only:
