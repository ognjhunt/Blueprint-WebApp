...
## Run Durham indoor public-location supply sourcing
...
- inputs:
  ...
  - city-launch-durham-nc-deep-research.md
  - ops/paperclip/playbooks/city-launch-durham-nc.md
  - Durham source policy
  - Durham city-opening brief
...
- done when:
  - Durham indoor supply names a broad set of concrete indoor-only or indoor-primary public-facing/common-access location candidates with source URLs, source queries, public-access posture, indoor posture, allowed capture zones, avoid zones, confidence, verification status, and review-only or promotion-ready state.
  - Durham rejected candidates are saved with reason codes and evidence instead of silently disappearing.
  - Durham evidence log records the search queries and fetched URLs used after reusing the city Deep Research artifact as a source map.
  - Durham review candidates are validated with the seed script, materialized into a durable candidate ledger or app-visible review feed, and automatically passed through deterministic public-space review for the newly written ids without marking them rights-cleared, operator-approved, payable, or derived-world-model-ready.
...
## Prepare the Durham Ad Studio claims review and creative handoff

- key: ad-studio-creative-handoff
- phase: demand
- agent owner: robot-team-growth-agent
- human owner: growth-lead
- purpose: Turn Durham proof-led positioning into draft-safe marketing assets without treating generated creative as captured site truth.
...
- inputs:
  - Durham city-opening brief
  - Durham exact-site buyer loop
  - ad_studio_runs
  - Durham 72h GTM contract
- done when:
  - Durham Ad Studio run exists with a claims ledger, claims review, prompt pack, and image execution handoff.
...
## Record Durham Meta Ads read-only proof and paused-draft readiness

- key: meta-paused-draft-readiness
- phase: demand
- agent owner: robot-team-growth-agent
- human owner: founder
- purpose: Keep Durham paid-acquisition prep auditable without allowing active ads or live spend to happen by implication.
...
- source: default_task_bundle
- inputs:
  - Durham Ad Studio run
  - meta_ads_cli_runs
  - Durham budget policy
  - Durham 72h GTM contract
- done when:
  - Durham Meta Ads CLI status/read-only proof is recorded or blocked with exact missing env/account fields.
...
## Persist the Durham 24/48/72h launch scorecard windows

- key: scorecard-window-24-48-72
- phase: measurement
- agent owner: analytics-agent
- human owner: growth-lead
- purpose: Make the Durham launch measurable at fixed 24h, 48h, and 72h checkpoints from first-party collections instead of relying on a current-only admin snapshot.
- policy_guardrail: none
...
- inputs:
  ...
  - Durham 72h GTM contract
- done when:
  - Durham 24h, 48h, and 72h scorecard artifact paths exist and name the exact Firestore/admin collection and query names used for each lane.
