...
## Run San Jose indoor public-location supply sourcing
...
- inputs:
  ...
  - city-launch-san-jose-ca-deep-research.md
  - ops/paperclip/playbooks/city-launch-san-jose-ca.md
  - San Jose source policy
  - San Jose city-opening brief
...
- done when:
  - San Jose indoor supply names a broad set of concrete indoor-only or indoor-primary public-facing/common-access location candidates with source URLs, source queries, public-access posture, indoor posture, allowed capture zones, avoid zones, confidence, verification status, and review-only or promotion-ready state.
  - San Jose rejected candidates are saved with reason codes and evidence instead of silently disappearing.
  - San Jose evidence log records the search queries and fetched URLs used after reusing the city Deep Research artifact as a source map.
  - San Jose review candidates are validated with the seed script, materialized into a durable candidate ledger or app-visible review feed, and automatically passed through deterministic public-space review for the newly written ids without marking them rights-cleared, operator-approved, payable, or derived-world-model-ready.
...
## Prepare the San Jose Ad Studio claims review and creative handoff

- key: ad-studio-creative-handoff
- phase: demand
- agent owner: robot-team-growth-agent
- human owner: growth-lead
- purpose: Turn San Jose proof-led positioning into draft-safe marketing assets without treating generated creative as captured site truth.
...
- inputs:
  - San Jose city-opening brief
  - San Jose exact-site buyer loop
  - ad_studio_runs
  - San Jose 72h GTM contract
- done when:
  - San Jose Ad Studio run exists with a claims ledger, claims review, prompt pack, and image execution handoff.
...
## Record San Jose Meta Ads read-only proof and paused-draft readiness

- key: meta-paused-draft-readiness
- phase: demand
- agent owner: robot-team-growth-agent
- human owner: founder
- purpose: Keep San Jose paid-acquisition prep auditable without allowing active ads or live spend to happen by implication.
...
- source: default_task_bundle
- inputs:
  - San Jose Ad Studio run
  - meta_ads_cli_runs
  - San Jose budget policy
  - San Jose 72h GTM contract
- done when:
  - San Jose Meta Ads CLI status/read-only proof is recorded or blocked with exact missing env/account fields.
...
## Persist the San Jose 24/48/72h launch scorecard windows

- key: scorecard-window-24-48-72
- phase: measurement
- agent owner: analytics-agent
- human owner: growth-lead
- purpose: Make the San Jose launch measurable at fixed 24h, 48h, and 72h checkpoints from first-party collections instead of relying on a current-only admin snapshot.
- policy_guardrail: none
...
- inputs:
  ...
  - San Jose 72h GTM contract
- done when:
  - San Jose 24h, 48h, and 72h scorecard artifact paths exist and name the exact Firestore/admin collection and query names used for each lane.
