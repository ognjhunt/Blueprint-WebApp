...
## Launch Thesis
One narrow exact-site hosted review motion targeting AMR integrators executing digital-twin simulations in multi-tenant 3PL logistics environments.
...
- evidence-backed: the target ledger mode for this city is `deep_research_records`. Research-backed named targets already available include US Cold Storage - Ambient Module.
...
- Choose the lawful access mode per target from: buyer_requested_site, site_operator_intro, capturer_existing_lawful_access.
...
Implement a 3-Tier multi-tenant consent packet (Property Owner, Facility Operator, Inventory Tenant) before authorizing capture dispatch.
...
- high: Verify exact-site artifact compatibility with NVIDIA Omniverse and iWAREHOUSE before live outreach. (validation required)
- high: Verify if export-controlled or dual-use industrial buyers mandate air-gapped delivery over Blueprint-WebApp. (validation required)
