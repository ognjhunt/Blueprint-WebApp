# Sacramento, CA — Blueprint City Demand Plan
...
- evidence-backed: current deep research names buyer targets such as Raymond West, Lineage Logistics, Locus Robotics.
...
- robot_team_inbound_captured: required_tracked — Emitted from inbound-request.ts on new robot_team inbound.
- proof_path_assigned: required_tracked — Emitted from admin-leads when qualification stamps a robot_team.
- proof_pack_delivered: required_tracked — Emitted from pipeline state machine when world_model_outputs >= 2 artifacts.
- hosted_review_ready: required_tracked — Emitted from pipeline state machine when hosted review readiness check passes.
- hosted_review_started: required_tracked — Emitted from pipeline state machine when runtime demo + launch URL exist.
- hosted_review_follow_up_sent: required_tracked — Emitted from pipeline state machine when hosted review started + evaluation artifacts present.
- human_commercial_handoff_started: required_tracked — Emitted from pipeline state machine when opportunity handoff or qualified export present.
- proof_motion_stalled: required_tracked — Emitted from pipeline state machine when qualification regresses or recapture required.
