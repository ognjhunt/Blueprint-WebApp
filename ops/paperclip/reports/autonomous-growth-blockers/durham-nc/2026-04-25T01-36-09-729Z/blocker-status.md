...
- generated_at: 2026-04-25T01:36:09.060Z
...
| Sender verification and durable reply resume must be production-proven | blocked | ops/runtime config | Run npm run human-replies:audit-durability, set the missing live env/provider state, then prove a tagged reply resumes execution. |
...
- stage_reached: public issueCounter=62, local issueCounter=4548, bridgeIssue=BLU-46
...
- stage_reached: emailTransport=configured, senderVerification=unknown, gmailProductionReady=false, watcherEnabled=false, missingEnv=8
...
- next_action: Run npm run human-replies:audit-durability, set the missing live env/provider state, then prove a tagged reply resumes execution.
- missing_env:
