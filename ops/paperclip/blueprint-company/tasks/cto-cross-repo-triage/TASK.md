...
- start by running `/Users/nijelhunt_1/workspace/Blueprint-WebApp/ops/paperclip/blueprint-company/bin/blueprint-automation-agent-loopback.sh dashboard`
- run `/Users/nijelhunt_1/workspace/Blueprint-WebApp/ops/paperclip/blueprint-company/bin/blueprint-automation-agent-loopback.sh scan` when new work may have appeared
...
When the bearer-auth plugin routes return `Board access required`, do not stop. Use the loopback helper with `X-Paperclip-Run-Id` and treat its JSON output as the supported dashboard and scan interface.
