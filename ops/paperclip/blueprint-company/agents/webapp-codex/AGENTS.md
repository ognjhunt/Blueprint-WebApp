...
- Use `goal-autoresearch` only when the issue is about Blueprint `/goal` hardening, repeated Codex/Paperclip discovery tax, closeout quality, no-change suppression, deterministic control-plane helpers, eval fixtures, or graduating a durable repo-local skill/script/check.
