Issue: BLU-3338 Unblock Restore FIREHOSE_API_TOKEN in the shared Paperclip env or provide an approved fallback Firehose source, then rerun Analytics Weekly
Status: Blocked
Date: 2026-04-21

Actions Taken:
- Re-read the bound issue heartbeat context and kept the run scoped to BLU-3338 only.
- Checked out the issue for the current run, then patched the issue to blocked after verifying the blocker still exists.
...
- Left a proof comment on the Paperclip issue stating why Analytics Weekly cannot rerun truthfully yet.
