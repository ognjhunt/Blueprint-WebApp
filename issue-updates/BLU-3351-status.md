Issue: BLU-3351 Unblock Restore FIREHOSE_API_TOKEN in the shared Paperclip env or provide an approved fallback Firehose source, then rerun Analytics Weekly
Status: Blocked
Date: 2026-04-21

Actions Taken:
- Re-read the bound heartbeat context and kept the run scoped to BLU-3351 only.
- Read the live issue record and confirmed the issue is assigned to this agent and still has no completed unblock path.
...
- Checked issue comments and found only an earlier runtime-rate-limit note; no new unblock context was added.
